...
grant connect, resource, dba to hr1; 
...
CREATE TABLE br_regions
    ( region_id      NUMBER 
       CONSTRAINT  br_region_id_nn NOT NULL 
    , region_name    VARCHAR2(25) 
    );

CREATE UNIQUE INDEX br_reg_id_pk
ON br_regions (region_id);

alter table BR_REGIONS enable row movement; 
...
CREATE TABLE br_countries 
    ( country_id      CHAR(2) 
       CONSTRAINT  br_country_id_nn NOT NULL 
    , country_name    VARCHAR2(40) 
    , region_id       NUMBER 
    , CONSTRAINT     br_country_c_id_pk 
...
CREATE TABLE br_locations
    ( location_id    NUMBER(4)
    , street_address VARCHAR2(40)
    , postal_code    VARCHAR2(12)
    , city       VARCHAR2(30)
	CONSTRAINT     br_loc_city_nn  NOT NULL
    , state_province VARCHAR2(25)
    , country_id     CHAR(2)
    ) ;

CREATE UNIQUE INDEX br_loc_id_pk
ON br_locations (location_id) ;

Rem 	Useful for any subsequent addition of rows to locations table
Rem 	Starts with 3300

CREATE SEQUENCE br_locations_seq
 START WITH     3300
 INCREMENT BY   100
 MAXVALUE       9900
 NOCACHE
...
CREATE TABLE br_departments
    ( department_id    NUMBER(4)
    , department_name  VARCHAR2(30)
	CONSTRAINT  br_dept_name_nn  NOT NULL
    , manager_id       NUMBER(6)
    , location_id      NUMBER(4)
    ) ;

CREATE UNIQUE INDEX br_dept_id_pk
ON br_departments (department_id) ;

Rem 	Useful for any subsequent addition of rows to departments table
Rem 	Starts with 280 

CREATE SEQUENCE br_departments_seq
 START WITH     280
 INCREMENT BY   10
 MAXVALUE       9990
 NOCACHE
...
CREATE TABLE br_jobs
    ( job_id         VARCHAR2(10)
    , job_title      VARCHAR2(35)
	CONSTRAINT     br_job_title_nn  NOT NULL
    , min_salary     NUMBER(6)
    , max_salary     NUMBER(6)
    ) ;

CREATE UNIQUE INDEX br_job_id_pk 
ON br_jobs (job_id) ;
...
CREATE TABLE br_employees
    ( employee_id    NUMBER(6)
    , first_name     VARCHAR2(20)
    , last_name      VARCHAR2(25)
	 CONSTRAINT     br_emp_last_name_nn  NOT NULL
    , email          VARCHAR2(25)
	CONSTRAINT     br_emp_email_nn  NOT NULL
    , phone_number   VARCHAR2(20)
    , hire_date      DATE
	CONSTRAINT     br_emp_hire_date_nn  NOT NULL
    , job_id         VARCHAR2(10)
	CONSTRAINT     br_emp_job_nn  NOT NULL
    , salary         NUMBER(8,2)
    , commission_pct NUMBER(2,2)
    , manager_id     NUMBER(6)
    , department_id  NUMBER(4)
    , CONSTRAINT     br_emp_salary_min
                     CHECK (salary > 0) 
    , CONSTRAINT     br_emp_email_uk
                     UNIQUE (email)
    ) ;

CREATE UNIQUE INDEX br_emp_emp_id_pk
ON br_employees (employee_id) ;
...
CREATE SEQUENCE br_employees_seq
 START WITH     207
 INCREMENT BY   1
 NOCACHE
 NOCYCLE;
...
CREATE TABLE br_job_history
    ( employee_id   NUMBER(6)
	 CONSTRAINT    br_jhist_employee_nn  NOT NULL
    , start_date    DATE
	CONSTRAINT    br_jhist_start_date_nn  NOT NULL
    , end_date      DATE
	CONSTRAINT    br_jhist_end_date_nn  NOT NULL
    , job_id        VARCHAR2(10)
	CONSTRAINT    br_jhist_job_nn  NOT NULL
    , department_id NUMBER(4)
    , CONSTRAINT    br_jhist_date_interval
                    CHECK (end_date > start_date)
    ) ;

CREATE UNIQUE INDEX br_jhist_emp_id_st_date_pk 
ON br_job_history (employee_id, start_date) ;
...
CREATE OR REPLACE VIEW br_emp_details_view
  (employee_id,
   job_id,
   manager_id,
   department_id,
...
INSERT INTO br_countries VALUES 
        ( 'IT'
        , 'Italy'
        , 1 
        );
...
INSERT INTO br_locations VALUES 
        ( 1000 
...
INSERT INTO br_departments VALUES 
        ( 10
        , 'Administration'
        , 200
        , 1700
...
INSERT INTO br_employees VALUES 
        ( 100
        , 'Steven'
        , 'King'
        , 'SKING'
