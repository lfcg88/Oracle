...
declare
     ...
     wuser varchar2 (15) := substr('&1',1,instr('&1','.',1,1)-1);
     wtable varchar2 (30)  := substr('&1',instr('&1','.',1,1)+1);
     /* Indexes */
     cursor cind is
...
begin
  ...
  for rind in cind loop
     wcount := wcount + 1;
       ...
       dbms_output.put_line('tablespace TBS_INTQ_DESENV'); --' || rind.tablespace_name);
