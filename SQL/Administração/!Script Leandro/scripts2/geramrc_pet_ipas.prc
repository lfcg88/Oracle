CREATE OR REPLACE PROCEDURE geramrc_pet_ipas AS

--Para catalogar 20/04/2011 INPI
--cat 24/04/2001 Bia
--comentado o
   ...
   CURSOR cSOLICITACAO_PET IS
      SELECT DISTINCT S.DATAHORA,
         ...
         --   AND trunc(S.DATAHORA) = to_date('11-03-2001', 'DD-MM-YYYY') ;  
         
         -- trunc(S.DATAHORA) = to_date('11-03-2011', 'DD-MM-YYYY');
...
--------Imprime dados do mrc_pet-----------------------------------------------------------------
    IF  vprocessoenviado = 'S' THEN
     UTL_FILE.PUT_LINE(FILEID,
                    ...
                    RPAD(TO_CHAR(rSOLICITACAO.DATAHORA, 'DD/MM/YYYY') ,10,' ') || /* Data do documento */
                    RPAD(TO_CHAR(rSOLICITACAO.DATAHORA, 'HH24:MI') ,5,' ') || /* Hora do documento */
                    RPAD(TO_CHAR(rSOLICITACAO.NRPROTOCOLO),12,' ') || /*Numero do protocolo*/
                    RPAD(TO_CHAR(vIDPROCURADOR),14,' ') || /*Numero do procurador*/
                    RPAD(TO_CHAR(vIDTIPOPROCURADOR),1,' ') || /*Tipo do procurador*/
                    RPAD(TO_CHAR(vIDTITULAR),14,' ') || /*Numero do requerente*/
                    RPAD(NVL(TO_CHAR(rSOLICITACAO.NRRPIGERADORA), ' '),4,' ') || /*Numero da RPI Geradora*/
                    LPAD(TO_CHAR(vNRGRU),16,'0') || /*Numero da GRU*/
                    RPAD(TO_CHAR(replace(NVL(rSOLICITACAO.TEXTOPETICAO,' '),chr(13)||chr(10),chr(35)||chr(36)||chr(37)||chr(38))),2500,' ')||/*Texto da Petic?o*/
                    lPAD(TO_CHAR(vCODDESPACHO),3,'0') || /* Codigo de despacho */
                    lPAD(NVL(TO_CHAR(vSITDESPACHO),'   '),3,'0') ||   /* Situacao */
                    RPAD(TO_CHAR(replace(NVL(vTEXTO,' '),chr(13)||chr(10),chr(35)||chr(36)||chr(37)||chr(38))),4000,' ')||/* Texto */
                    RPAD(TO_CHAR(replace(NVL(rSOLICITACAO.despachointerno,' '),chr(13)||chr(10),chr(35)||chr(36)||chr(37)||chr(38))),4000,' ')||
                    RPAD(TO_CHAR(vqtd3),5,' ')||
                    RPAD(TO_CHAR(vqtd),5,' ')||
  ...
  -- Escreve arquivo MRC_PETDOC --
   ...
   --FILEID1  := UTL_FILE.FOPEN ('/home/oracle/arqstxt', NOMEARQ1, 'W',32767);
  ...
  BEGIN
    ...
    --FILEID2  := UTL_FILE.FOPEN ('/home/oracle/arqstxt', NOMEARQ2, 'W',32767);
 ...
 --   end loop;
  ...
  BEGIN
    ...
    --FILEID3  := UTL_FILE.FOPEN ('/home/oracle/arqstxt', NOMEARQ3, 'W',32767);
