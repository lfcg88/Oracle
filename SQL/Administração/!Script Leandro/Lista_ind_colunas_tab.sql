...
Accept wrk_nome_obj prompt "Digite o Owner.Tabela do índice : "
...
SELECT INDS.TABLE_OWNER||'.'|| INDS.TABLE_NAME  TABLE_NAME
 ...
 WHERE COLS.INDEX_OWNER = INDS.OWNER
   ...
   AND INDS.TABLE_NAME  =  UPPER(substr('&wrk_nome_obj',instr('&wrk_nome_obj','.',1,1)+1))
   AND INDS.OWNER = UPPER(substr('&wrk_nome_obj',1,instr('&wrk_nome_obj','.',1,1)-1))
