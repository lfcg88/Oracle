...
CREATE OR REPLACE
package body gen_tab_struct is
   ...
   begin
      ...
      if c_tab_columns%notfound then
         ...
         loop
            ...
            if SaveCursor.data_type = 'VARCHAR2' or SaveCursor.data_type = 'RAW' or
         ...
         -- Pct Free/Used
         --
         sLineOut := 'PCTFREE &'||'5'||' PCTUSED &'||'6';
         PutLine(f_file,sLineOut);
         ExecCommand(f_file);
         sLineOut := 'GRANT SELECT ON '||s_table||
            ' TO rle_access';
         PutLine(f_file,sLineOut);
         ExecCommand(f_file);
...
--
   ...
   -- Variaveis
   --
   sLineOut varchar2(10000);
   sLineAux varchar2(10000);
   bFirstTime boolean;
   bFecha boolean;
   sOutFileName varchar2(40);
   sDirect Varchar2(200);
