...
# http://www.think-forward.com/sql/mem.htm
...
echo "" > $ShadUse
for i in $ShadPrc;do
   $Pmap $i | grep "read/write" | grep -v shared | awk '{ print $2 }' | awk -FK
'{ print $1 }' >> $ShadUse
done
...
if [ $ParmFound = "0" ];then
   ...
   $Pmap $2 | grep "read/write" | grep -v shared | awk '{ print $2 }' | awk -FK
'{ print $1 }' >> $PidPUse
...
$PidSUse
   $Pmap $2 | grep "shared" | awk '{ print $2 }' | awk -FK '{ print $1 }' >> $Pi
dSUse
