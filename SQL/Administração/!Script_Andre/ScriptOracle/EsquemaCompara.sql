...
(The script does not do a complete check, it does not for example
...
If you find ways to improve this script or have any comments and/or
problems, please send e-mail to the author.
...
accept REMOTEPASSW char prompt 'Enter remote password:' 
accept connstring char prompt 'Enter remote connectstring:'
spool c:\dbdiff_p2cc_t1pr
...
PROMPT         remote schema: &remoteschema       
PROMPT  ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
PROMPT
PROMPT


-- create database link bscst1me connect to &REMOTESCHEMA identified 
-- by &REMOTEPASSW using '&CONNSTRING';

-- create PUBLIC database link DSV8.WORLD connect to "OWNCSEG" identified 
-- by "2002" using 'dsv6';
...
select table_name from user_TAbles@DSV8.WORLD
intersect
...
select table_name,column_name from user_tab_columns@DSV8.WORLD
where table_name in
(select table_name from &prex.common_tables)
minus
...
select
 ...
 l.DEFAULT_LENGTH ,                    
 l.DATA_DEFAULT  
from user_tab_columns@DSV8.WORLD l,&prex.common_tables c
where c.table_name=l.table_name ;
TYPE rec is record (
 TABLE_NAME                   VARCHAR2(30),
 COLUMN_NAME                VARCHAR2(30),
...
select table_name,index_name from user_indexes@DSV8.WORLD
where table_name in
(select table_name from &prex.common_tables)
minus
...
select * from &prex.ind1;


create table &prex.ind2 as
select table_name,index_name from user_indexes
where table_name in
(select table_name from &prex.common_tables)
minus
...
select * from &prex.ind2;
create table &prex.ind as select * from &prex.ind1 
intersect select * from &prex.ind2;

PROMPT
PROMPT
PROMPT  ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
PROMPT DIFFERENCES IN INDEX DEFINITIONS
PROMPT  ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

select a.index_name,a.column_position,a.column_name,b.column_name from
&prex.ind2 q, user_ind_columns a, user_ind_columns@DSV8.WORLD b where
q.index_name=a.index_name and 
a.index_name=b.index_name and a.column_position=b.column_position(+) and
a.column_name != b.column_name(+)
union
select a.index_name,a.column_position,a.column_name,b.column_name from
&prex.ind2 q, user_ind_columns@DSV8.WORLD a, user_ind_columns b where
q.index_name=a.index_name and 
a.index_name=b.index_name and a.column_position=b.column_position(+) and
a.column_name != b.column_name(+)
order by 1,2
...
select table_name,constraint_name from user_constraints@DSV8.WORLD
...
from user_constraints a, user_constraints@DSV8.WORLD b where
...
user_constraints a, user_constraints@DSV8.WORLD b,
(select z.constraint_name from 
(select constraint_name, table_name from useR_constraints union
select constraint_name, table_name from user_constraints@DSV8.WORLD) z
...
intersect 
select constraint_name, constraint_type, table_name 
from user_constraints@DSV8.WORLD;
...
union
select a.constraint_name,a.position,a.column_name,b.constraint_type 
from user_cons_columns@DSV8.WORLD a, &prex.comcons b
where a.constraint_name=b.constraint_name
minus
(select a.constraint_name,a.position,a.column_name,b.constraint_type 
   from user_cons_columns a, &prex.comcons b
...
for c in c1 loop
   ...
   for c3 in (select column_name col 
             from user_cons_columns@DSV8.WORLD 
...
for c in c1 loop
  select search_condition into search1 from user_constraints 
   where constraint_name=c.constraint_name;
  select search_condition into search2 from user_constraints@DSV8.WORLD 
   where constraint_name=c.constraint_name;
...
select trigger_name from user_Triggers@DSV8.WORLD;
...
select trigger_name from user_Triggers@DSV8.WORLD minus 
select trigger_name from user_Triggers@DSV8.WORLD;
...
loop
 ...
 if replace(desc1,chr(10),'') != replace(desc2,chr(10),'') then
  ...
  if body1 != body2 then
    dbms_output.put_line('   ');
    dbms_output.put_line('THE PL/SQL BLOCKS ARE DIFFERENT! (run trigger.sql)');
    dbms_output.put_line('   ');
  end if;
...
select distinct name,type from user_source@DSV8.WORLD;
...
select distinct name,type from user_source@DSV8.WORLD minus 
...
select a.name,a.type,a.line,a.text 
 from user_source@DSV8.WORLD a, &prex.comcod b 
where a.name=b.name 
minus
(select a.name,a.type,a.line,a.text 
   from user_source a, &prex.comcod b 
...
select view_name from user_views@DSV8.WORLD
intersect
...
dbms_output.enable(100000);
for c in c1 loop
  select text into def1 from user_Views where view_name=c.view_name;
  select text into def2 from user_Views@DSV8.WORLD where view_name=c.view_name;
  if def1 != def2 then
