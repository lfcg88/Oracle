...
function COOLjsTreeNodePRO( treeView, parentNode , text, url, target){
    ...
    this.getContent=function( ){
		function itemSquare(node){
			...
			return "<td valign=bottom valign=\"bottom\" width=\""+node.wbtn+"\"><img id=\""+node.id()+"nf\" name=\""+node.id()+"nf\" src=\"" + img + "\" width="+node.wbtn+" height="+node.hbtn+" border=0></td>\n";
		}
		...
		function blankSquares(node, ww, ll){
			if (node.treeView.fmt.exp){
				...
				for (i=ll;i>0;i--){
					...
					res="<td width=\""+w+"\" valign=bottom><img src=\"" + img + "\" width="+w+" height="+h+" border=0></td>" + res;
				}
				return res;
			}else{
				var img=node.treeView.fmt.iE;
				idn==0?hh = 30:hh = 1;
				return "<td width=\""+ww+"\" valign=bottom><img src=\"" + img + "\" width="+ww+" height=" + hh + " border=0></td>\n";
			}
		}
	...
	if ( this.treeView.fmt.showB || this.treeView.fmt.exp) s +=this.treeView.fmt.exp ? buttonSquare(this) : (this.hasChildren() && idn > 0 ?  buttonSquare(this) : blankSquares(this, this.treeView.fmt.Ew));		
		...
		s +=this.treeView.ns4?'<td nowrap=\"1\" valign=bottom><ilayer id="'+this.id()+'a">'+t+'</ilayer></td></tr></table>' : '<td nowrap=\"1\" valign=\"bottom\"><div id="'+this.id()+'a">'+t+'</div></td></tr></table>';
		return s;
	};
