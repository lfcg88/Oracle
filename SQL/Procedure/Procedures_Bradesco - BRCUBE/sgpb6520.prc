CREATE OR REPLACE PROCEDURE SGPB_PROC.SGPB6520 IS
---------------------------------------------------------------------------------------------------------------------------
...
VAR_CROTNA                 ARQ_TRAB.CROTNA%TYPE               	:= 'SGPB6520';
...
VAR_CPARM                  PARM_CARGA.CPARM%TYPE              	:= 754; -- PARAMETRO DE CARGA PARA OS DADOS DE REDE
...
BEGIN
   ...
   END LOOP;
   --
   VAR_LOG := 'TOTAL DE REGISTROS ADICIONADOS: '|| TO_CHAR(VAR_TOT_REG_LIDO_BODY);
   PR_GRAVA_MSG_LOG_CARGA(VAR_CROTNA, VAR_LOG, VAR_LOG_PROCESSO, NULL, NULL);
   COMMIT;
   --   
EXCEPTION
