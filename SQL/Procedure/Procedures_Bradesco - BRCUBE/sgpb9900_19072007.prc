CREATE OR REPLACE PROCEDURE SGPB_PROC.SGPB9900_19072007 IS
  -------------------------------------------------------------------------------------------------
  ...
  VAR_FAIXA_INI_FN    NUMBER(10) := 870006;   --FAIXA FINASA
  VAR_FAIXA_FIM_FN    NUMBER(10) := 879999;   --FAIXA FINASA
  ------------------------------------------------------------------------------------------------
  --CURSOR
...
BEGIN
   ...
   --
       ...
       OPEN C_CORRETOR_CCC FOR
          ...
          SELECT APC.CUND_PROD,     --chave apolc_prod_crrtr
            ...
            JOIN MPMTO_AG_CRRTR MAC
              ON MAC.CUND_PROD = C.CUND_PROD
             AND MAC.CCRRTR_DSMEM = C.CCRRTR
             AND MAC.CCRRTR_ORIGN BETWEEN VAR_FAIXA_INI_FN AND VAR_FAIXA_FIM_FN; --faixa banco
            --
            --WHERE ARP.CGRP_RAMO_PLANO = 120;
       ...
       FETCH C_CORRETOR_CCC INTO int_cund_prod,
        ...
        --DWSCHEDULER
        PR_GRAVA_MSG_LOG_CARGA(VAR_ROTINA,'FINASA APOLICE PRODUCAO DO CORRETOR '||VAR_CPF_CNPJ_BASE||'DELETADA - COMPETENCIA :'||I.COMPETENCIA,'P',NULL,NULL);
        COMMIT;
        --
       ...
       OPEN C_CORRETOR_CCC FOR
          ...
          SELECT pc.cgrp_ramo_plano, --chave prod_crrtr
            ...
            JOIN MPMTO_AG_CRRTR MAC ON MAC.CUND_PROD = PC.CUND_PROD
                                   AND MAC.CCRRTR_DSMEM = PC.CCRRTR
                                   AND MAC.CCRRTR_ORIGN BETWEEN VAR_FAIXA_INI_FN AND VAR_FAIXA_FIM_FN
            JOIN CRRTR C
              ON C.CCRRTR = PC.CCRRTR
             AND C.CUND_PROD = PC.CUND_PROD
             AND C.CCPF_CNPJ_BASE = VAR_CPF_CNPJ_BASE
       ...
       FETCH C_CORRETOR_CCC INTO int_cgrp_ramo_plano,
        ...
        --DWSCHEDULER
        PR_GRAVA_MSG_LOG_CARGA(VAR_ROTINA,'FINASA PRODUCAO DO CORRETOR '||VAR_CPF_CNPJ_BASE||'DELETADA - COMPETENCIA :'||I.COMPETENCIA,'P',NULL,NULL);
        COMMIT;
        --
       ...
       ----------------------------------------------------
        ...
        --
          ...
          --
            ...
            JOIN AGPTO_RAMO_PLANO ARP ON ARP.CGRP_RAMO_PLANO IN (PC_UTIL_01.Auto)
                                     AND ARP.CRAMO = VAT.cramo
          --
           WHERE VAT.ccrrtr BETWEEN VAR_FAIXA_INI_FN AND VAR_FAIXA_FIM_FN
             -------------?????????????xxxxxxxxxxx????????????
             ----AND vat.demis_endss = intrdia
             AND EXISTS (SELECT 1
                    FROM CRRTR C
        ...
        --DWSCHEDULER
        PR_GRAVA_MSG_LOG_CARGA(VAR_ROTINA,'FINASA APOLICE AUTO PRODUCAO DO CORRETOR '||VAR_CPF_CNPJ_BASE||'CARREGADA - COMPETENCIA :'||I.COMPETENCIA,'P',NULL,NULL);
        COMMIT;
        --
        
       ----------------------------------------------------
       --CARREGANDO AS APOLICES DO CORRETOR X / COMPETENCIA Y
       --sgpb0136 - extra-banco - re --> CN para todos
       ----------------------------------------------------
        ...
        INSERT INTO APOLC_PROD_CRRTR (CUND_PROD,CCIA_SEGDR,CRAMO_APOLC,CAPOLC,CITEM_APOLC,CTPO_DOCTO,CENDSS_APOLC,DEMIS_APOLC,
          ...
          SELECT VRE.CSUCUR,
                 ...
                 'CN',
                 VRE.CCRRTR,
                 max(VRE.DINIC_VGCIA_DOCTO),
                 max(VRE.DFIM_VGCIA_APOLC),
                 sum(VRE.VPRMIO_EMTDO_CSSRO_CDIDO),
                 VRE.CCHAVE_DOCTO_PRMIO,
                 sysdate,
                 'N',
                 'N'      
          ...
          --
           WHERE VRE.CCRRTR BETWEEN VAR_FAIXA_INI_EX AND VAR_FAIXA_FIM_EX
             --XXXXXXXXXXXX
        ...
        --DWSCHEDULER
        PR_GRAVA_MSG_LOG_CARGA(VAR_ROTINA,'EXTRA-BANCO APOLICE RE PRODUCAO DO CORRETOR '||VAR_CPF_CNPJ_BASE||'CARREGADA - COMPETENCIA :'||I.COMPETENCIA,'P',NULL,NULL);
        COMMIT;
        --
        
       ----------------------------------------------------
       --CARREGANDO AS APOLICES DO CORRETOR X / COMPETENCIA Y
       --sgpb0137 - banco e finasa - re --> CN para todos
       ----------------------------------------------------
        --?????????????????
       ...
       FOR c IN (SELECT I.COMPETENCIA ccompt_prod,
                   ...
                   JOIN CRRTR C
                    ...
                    --AND C.Ccrrtr BETWEEN VAR_FAIXA_INI and VAR_FAIXA_FIM
                    ---????????????XXXXXXXXXXX
                    AND C.CCPF_CNPJ_BASE = VAR_CPF_CNPJ_BASE
                     --
                  WHERE APC.DEMIS_APOLC BETWEEN to_date(I.COMPETENCIA,'YYYYMM') AND
                   	...
                   	AND ARP.CGRP_RAMO_PLANO IN (pc_util_01.Auto, pc_util_01.Re, pc_util_01.ReTodos) --apenas auto
        ...
        insertUpdate(c.ccompt_prod,c.cund_prod,c.ccrrtr,c.cgrp_ramo_plano,c.ctpo_comis,c.qtot_item_prod,nvl(c.vprod_crrtr,0));
                ...
                IF (C.CGRP_RAMO_PLANO IN (pc_util_01.Auto, pc_util_01.Re)) THEN
                 ...
                 LOOP
                  	...
                  	IF (SQL%ROWCOUNT = 0) THEN
                  	    ROLLBACK;
                     		PR_GRAVA_MSG_LOG_CARGA(VAR_ROTINA,'ERRO. O UPDATE NAO AFETOU NENHUMA LINHA.',pc_util_01.VAR_LOG_PROCESSO,NULL,NULL);
                     		PR_ATUALIZA_STATUS_ROTINA(VAR_ROTINA,VAR_PARAMETRO,PC_UTIL_01.VAR_ROTNA_PE);
                     		COMMIT;
