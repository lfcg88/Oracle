CREATE OR REPLACE PROCEDURE SGPB_PROC.SGPB6704 IS
-------------------------------------------------------------------------------------------------
--      BRADESCO SEGUROS S.A.
--      DATA            : 24/02/2008
--      AUTOR           : FABIO GIGLIO - VALUE TEAM
--      PROGRAMA        : SGPB6704.SQL
...
VAR_CROTNA                 	ARQ_TRAB.CROTNA%TYPE        	    := 'SGPB6704';
--
...
/*---------------------------------------------------------------------------------------------*/
 BEGIN
   VAR_CRGNAL	 		:= SUBSTR(VAR_REGISTRO_ARQUIVO, 11, 5);
 EXCEPTION
   WHEN OTHERS THEN
--
      VAR_CRGNAL			:= NULL;
      ...
      VAR_ERRO 	     	  	:= 'S';
--
      VAR_LOG := 'PROBLEMA AO CARREGAR CRGNAL DO ARQUIVO.'||
                 ' -- CCAMPA_DSTAQ: '||SUBSTR(VAR_REGISTRO_ARQUIVO, 1, 6)||
                 ' -- CPARM_HIERQ_DSTAQ: '||SUBSTR(VAR_REGISTRO_ARQUIVO, 7, 2)||
                 ' -- CCANAL_PROD_DW: '||SUBSTR(VAR_REGISTRO_ARQUIVO, 9, 2)||
                 ' -- VAR_CRGNAL: '||SUBSTR(VAR_REGISTRO_ARQUIVO, 11, 5)||
...
/*---------------------------------------------------------------------------------------------*/
 BEGIN
   VAR_CTPO_PSSOA	 		:= SUBSTR(VAR_REGISTRO_ARQUIVO, 16, 1);
 EXCEPTION
   WHEN OTHERS THEN
--
      VAR_CTPO_PSSOA		:= NULL;
...
--
      VAR_LOG := 'PROBLEMA AO CARREGAR CTPO_PSSOA DO ARQUIVO.'||
                 ...
                 ' -- CCANAL_PROD_DW: '||SUBSTR(VAR_REGISTRO_ARQUIVO, 9, 2)||
                 ' -- VAR_CRGNAL: '||SUBSTR(VAR_REGISTRO_ARQUIVO, 11, 5)||
                 ' -- CTPO_PSSOA: '||SUBSTR(VAR_REGISTRO_ARQUIVO, 16, 1)||
                 ' -- ERRO ORACLE: '||SUBSTR(SQLERRM, 1, 120);
      PR_GRAVA_MSG_LOG_CARGA(VAR_CROTNA, VAR_LOG, VAR_LOG_DADO, NULL, NULL);
      --DBMS_OUTPUT.PUT_LINE(SUBSTR(VAR_LOG,1,255));  
--
...
/*---------------------------------------------------------------------------------------------*/
 BEGIN
   VAR_CCPF_CNPJ_BASE 		:= TO_NUMBER(SUBSTR(VAR_REGISTRO_ARQUIVO, 17, 9));
 EXCEPTION
   WHEN OTHERS THEN
--
...
/*---------------------------------------------------------------------------------------------*/
 BEGIN
   ...
   WROW.VMETA_AUTO		:= TO_NUMBER(SUBSTR(VAR_REGISTRO_ARQUIVO, 26, 15))/100;
 EXCEPTION
   WHEN OTHERS THEN
--
...
/*---------------------------------------------------------------------------------------------*/
 BEGIN
   ...
   WROW.VMETA_RE			:= TO_NUMBER(SUBSTR(VAR_REGISTRO_ARQUIVO, 41, 15))/100;
 EXCEPTION
   WHEN OTHERS THEN
--
      WROW.VMETA_RE			:= NULL;
