create or replace procedure sgpb_proc.SGPB0151_ALTERADA(intrCodCanal  in CANAL_VDA_SEGUR.CCANAL_VDA_SEGUR %TYPE,
 ...
 IS
  ...
  -- codigo novo (nao presente em producao em 15/06/2007)
  -- ----------------------------------------------------
  ...
  -- variavel criada em 13/06/2007
  VAR_PROD_MINIMA NUMBER; 
  ----------------------------------------------------------------------------------------
  --RECUPERANDO O VALOR DO OBJETIVO RE (nao existente em producao em 15/06/2007)
  ----------------------------------------------------------------------------------------
  ...
  -- -------------------------------------------------------------
  -- final do codigo novo (nao presente em producao em 15/06/2007)
  -- -------------------------------------------------------------
  ----------------------------------------------------------------------------------------
...
BEGIN
    ...
    -- wassily
      ...
      IF VAR_TRIMESTRE <= intrTRIMESTRE THEN
        chrLocalErro := 95;
        --PR_GRAVA_MSG_LOG_CARGA_SGPB('SGPB0151','Cod.Erro: '||chrLocalErro ||' VAR_TRIMESTRE: '||
        --                 VAR_TRIMESTRE ||' intrTRIMESTRE: '||intrTRIMESTRE,pc_util_01.VAR_LOG_PROCESSO,NULL, NULL);
        -- COMMIT;                      
        for i in 0..2
        loop
        	PEL_AUTO(i)    :=0;
      ...
      -- Na definição do objetivo do auto deveria colocar o valor minimo da produção a ser apurado, se a produção fosse menor que
      -- esse minimo. Esse valor mínimo está definido da tabela PARM_CANAL_VDA_SEGUR e é o atributo VMIN_PROD_APURC (por canal).
      -- Essa proposta vale para todos os canais e todos os ramos.
      -- Ass. Wassily
