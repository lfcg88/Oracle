...
VAR_CROTNA          			ARQ_TRAB.CROTNA%TYPE  	  := 'SGPB6754';	-- parâmetro de rotina
---
...
BEGIN
	...
	VAR_SQL	:= 'SELECT POSIC_DSTAQ.ROWID
		     , (VPROD_AUTO + VPROD_RE) VPROD_CRRTR 
		     , RANK() OVER (PARTITION BY ';
				     
	IF PAR_CPARM_HIERQ_DSTAQ IN (PC_PARAMETROS_DESTAQUE.CPARM_HIERQ_DSTAQ_EX_BCO,			--EXTRA BANCO
								 PC_PARAMETROS_DESTAQUE.CPARM_HIERQ_DSTAQ_BCO_CNPJ) THEN    --BANCO CNPJ
		IF CIND_RGNAL THEN	
			VAR_SQL := VAR_SQL || PC_PARAMETROS_DESTAQUE.FC_RCUPD_HIERQ_EXTRA_BCO_RGNAL(VAR_CCAMPA_DSTAQ);
			VAR_SQL	:= VAR_SQL || '  ORDER BY CIND_ALCAN_META DESC, (VPROD_AUTO + VPROD_RE) DESC) NRKING_PROD_RGNAL ';
			VAR_SQL := VAR_SQL || '	, RANK() OVER (PARTITION BY ';
			VAR_SQL := VAR_SQL || PC_PARAMETROS_DESTAQUE.FC_RCUPD_HIERQ_EXTRA_BCO_RGNAL(VAR_CCAMPA_DSTAQ);
			VAR_SQL := VAR_SQL || ' ORDER BY CIND_ALCAN_META DESC, VPERC_CRSCT DESC) NRKING_PERC_CRSCT_RGNAL ';
		ELSE
			VAR_SQL := VAR_SQL || PC_PARAMETROS_DESTAQUE.FC_RCUPD_HIERQ_EXTRA_BCO(VAR_CCAMPA_DSTAQ);
			VAR_SQL	:= VAR_SQL || '  ORDER BY CIND_ALCAN_META DESC, (VPROD_AUTO + VPROD_RE) DESC) NRKING_PROD ';
			VAR_SQL	:= VAR_SQL || ', RANK() OVER (PARTITION BY ';
			VAR_SQL	:= VAR_SQL || PC_PARAMETROS_DESTAQUE.FC_RCUPD_HIERQ_EXTRA_BCO(VAR_CCAMPA_DSTAQ);
			VAR_SQL	:= VAR_SQL || ' ORDER BY CIND_ALCAN_META DESC, VPERC_CRSCT DESC) NRKING_PERC_CRSCT';			
		END IF;
	/*ELSIF PAR_CPARM_HIERQ_DSTAQ =  THEN --BANCO CNPJ
		VAR_SQL	:= VAR_SQL || PC_PARAMETROS_DESTAQUE.FC_RCUPD_HIERQ_BCO_CNPJ(VAR_CCAMPA_DSTAQ);
		VAR_SQL	:= VAR_SQL || '  ORDER BY CIND_ALCAN_META DESC, (VPROD_AUTO + VPROD_RE) DESC) NRKING_PROD ';
		VAR_SQL	:= VAR_SQL || '	, RANK() OVER (PARTITION BY ';
		VAR_SQL	:= VAR_SQL || PC_PARAMETROS_DESTAQUE.FC_RCUPD_HIERQ_BCO_CNPJ(VAR_CCAMPA_DSTAQ);
		VAR_SQL	:= VAR_SQL ||' ORDER BY CIND_ALCAN_META DESC, VPERC_CRSCT DESC) NRKING_PERC_CRSCT	';*/
	ELSIF PAR_CPARM_HIERQ_DSTAQ = PC_PARAMETROS_DESTAQUE.CPARM_HIERQ_DSTAQ_BCO_AG THEN --BANCO ANGENCIA
		IF CIND_RGNAL THEN
			VAR_SQL	:= VAR_SQL || PC_PARAMETROS_DESTAQUE.FC_RCUPD_HIERQ_BCO_AG_RGNAL(VAR_CCAMPA_DSTAQ);
			VAR_SQL	:= VAR_SQL || '  ORDER BY CIND_ALCAN_META DESC, (VPROD_AUTO + VPROD_RE) DESC) NRKING_PROD_RGNAL ';
			VAR_SQL	:= VAR_SQL || '	, RANK() OVER (PARTITION BY ';
			VAR_SQL	:= VAR_SQL || PC_PARAMETROS_DESTAQUE.FC_RCUPD_HIERQ_BCO_AG_RGNAL(VAR_CCAMPA_DSTAQ);
			VAR_SQL	:= VAR_SQL || ' ORDER BY CIND_ALCAN_META DESC, VPERC_CRSCT DESC) NRKING_PERC_CRSCT_RGNAL';
		ELSE
			...
			VAR_SQL	:= VAR_SQL || ' ORDER BY CIND_ALCAN_META DESC, VPERC_CRSCT DESC) NRKING_PERC_CRSCT ';
		END IF;	
	ELSIF PAR_CPARM_HIERQ_DSTAQ = PC_PARAMETROS_DESTAQUE.CPARM_HIERQ_DSTAQ_FNASA THEN --FINASA
	...
	VAR_SQL	:= VAR_SQL || ' FROM POSIC_DSTAQ 
			 , HIERQ_PBLIC_ALVO 
			 WHERE POSIC_DSTAQ.DAPURC_DSTAQ = TO_DATE('''|| TO_CHAR(VAR_DPROX_CARGA,'DDMMYYYY')||''',''DDMMYYYY'')'||
			 ' AND POSIC_DSTAQ.CCAMPA_DSTAQ = '||VAR_CCAMPA_DSTAQ||
			 ' AND POSIC_DSTAQ.CCANAL_PROD_DW = '||PAR_CANAL||
			 ' AND POSIC_DSTAQ.CPARM_HIERQ_DSTAQ = '||PAR_CPARM_HIERQ_DSTAQ||
			 ' AND POSIC_DSTAQ.CCAMPA_DSTAQ				= HIERQ_PBLIC_ALVO.CCAMPA_DSTAQ
			   AND POSIC_DSTAQ.CPARM_HIERQ_DSTAQ			= HIERQ_PBLIC_ALVO.CPARM_HIERQ_DSTAQ
			   AND POSIC_DSTAQ.CHIERQ_PBLIC_ALVO_DSTAQ	= HIERQ_PBLIC_ALVO.CHIERQ_PBLIC_ALVO_DSTAQ
			   AND POSIC_DSTAQ.CCANAL_PROD_DW			= HIERQ_PBLIC_ALVO.CCANAL_PROD_DW
			   AND (CIND_FALTA_META IS NULL OR CIND_FALTA_META = ''N'')
			   AND (POSIC_DSTAQ.CIND_BLOQ_CAMPA IS NULL OR POSIC_DSTAQ.CIND_BLOQ_CAMPA = ''N'')
			 ORDER BY VPROD_CRRTR DESC';
			 
	VAR_LOG  := SUBSTR('------O SELECT SERA:------'||VAR_SQL, 1, 990);
	PR_GRAVA_MSG_LOG_CARGA(VAR_CROTNA, VAR_LOG, VAR_TPO_LOG_PROCESSO, NULL, NULL);
	VAR_LOG  := SUBSTR('------O SELECT SERA:------'||VAR_SQL, 991);
	PR_GRAVA_MSG_LOG_CARGA(VAR_CROTNA, VAR_LOG, VAR_TPO_LOG_PROCESSO, NULL, NULL);
	COMMIT;
EXCEPTION
	WHEN OTHERS THEN
		VAR_LOG  := 'ERRO NO SUB-PROGRAMA MONTA_SQL.'||SUBSTR(SQLERRM(SQLCODE), 1, 200);
		PR_GRAVA_MSG_LOG_CARGA(VAR_CROTNA, VAR_LOG, VAR_TPO_LOG_PROCESSO, NULL, NULL);
		VAR_CSIT_CTRLM := 5;
		RAISE VAR_FIM_PROCESSO_ERRO;		
END MONTA_SQL;
...
VAR_NRKING_PERC_CRSCT	POSIC_DSTAQ.NRKING_PROD%TYPE;
...
BEGIN
	...
	LOOP
		FETCH CUR_RANK INTO VAR_ROWID, VAR_VPROD_CRRTR, VAR_NRKING_PROD, VAR_NRKING_PERC_CRSCT;
			
		UPDATE POSIC_DSTAQ
		SET NRKING_PROD 		= VAR_NRKING_PROD
		, NRKING_PERC_CRSCT		= VAR_NRKING_PERC_CRSCT
...
EXCEPTION
	WHEN OTHERS THEN
		VAR_LOG  := 'ERRO NO SUB-PROGRAMA GRAVA_RANK.'||SUBSTR(SQLERRM(SQLCODE), 1, 200);
		PR_GRAVA_MSG_LOG_CARGA(VAR_CROTNA, VAR_LOG, VAR_TPO_LOG_PROCESSO, NULL, NULL);
		VAR_CSIT_CTRLM := 5;
		RAISE VAR_FIM_PROCESSO_ERRO;
END GRAVA_RANK;
...
VAR_NRKING_PERC_CRSCT_RGNAL	POSIC_DSTAQ.NRKING_PROD%TYPE;
TYPE TP_CURSOR				IS REF CURSOR;
CUR_RANK					TP_CURSOR;
BEGIN
	OPEN CUR_RANK FOR VAR_SQL;
	...
	LOOP
		FETCH CUR_RANK INTO VAR_ROWID, VAR_VPROD_CRRTR,VAR_NRKING_PROD_RGNAL,VAR_NRKING_PERC_CRSCT_RGNAL;
			
		UPDATE POSIC_DSTAQ
		SET NRKING_PROD_RGNAL 		= VAR_NRKING_PROD_RGNAL 
		, NRKING_PERC_CRSCT_RGNAL 	= VAR_NRKING_PERC_CRSCT_RGNAL
	 	WHERE ROWID = VAR_ROWID;
...
EXCEPTION
	WHEN OTHERS THEN
		VAR_LOG  := 'ERRO NO SUB-PROGRAMA GRAVA_RANK_RGNAL.'||SUBSTR(SQLERRM(SQLCODE), 1, 200);
		PR_GRAVA_MSG_LOG_CARGA(VAR_CROTNA, VAR_LOG, VAR_TPO_LOG_PROCESSO, NULL, NULL);
		VAR_CSIT_CTRLM := 5;
		RAISE VAR_FIM_PROCESSO_ERRO;
...
BEGIN
	...
	FOR REG_PHD	IN (SELECT CPARM_HIERQ_DSTAQ,CCANAL_PROD_DW, IPARM_HIERQ_DSTAQ
		...
		VAR_LOG  := '---INICIA A CLASSIFICACAO DOS CORRETORES DA CAMPANHA - '||VAR_CCAMPA_DSTAQ
	 	...
	 	IF	(REG_PHD.CPARM_HIERQ_DSTAQ IN 
	 				(PC_PARAMETROS_DESTAQUE.CPARM_HIERQ_DSTAQ_EX_BCO 			--CANAL BANCO EXTRA BANCO
	 				,PC_PARAMETROS_DESTAQUE.CPARM_HIERQ_DSTAQ_BCO_AG 			--CANAL BANCO AGENCIA
	 				,PC_PARAMETROS_DESTAQUE.CPARM_HIERQ_DSTAQ_BCO_CNPJ)) THEN	--CANAL BANCO CNPJ 
	 		--PARA ESSAS HIERARQUIA EXISTE AGRUPAMENTO POR REGIONAL E GRUPO DE REGIONAL
...
EXCEPTION
	WHEN OTHERS THEN
    	VAR_LOG  := 'ERRO AO CARREGA RANK DE PRODUÇÃO. '||
     				SUBSTR(SQLERRM(SQLCODE), 1, 200);
        PR_GRAVA_MSG_LOG_CARGA(VAR_CROTNA, VAR_LOG, VAR_TPO_LOG_PROCESSO, NULL, NULL);
        VAR_CSIT_CTRLM := 5;
        RAISE VAR_FIM_PROCESSO_ERRO;
...
BEGIN
	...
	VAR_LOG  := 'INICIA O CALCULO DO PERCENTUAL DE CRESCIMENTO. PARA A CAMPANHA - '||VAR_CCAMPA_DSTAQ||
				' APURAÇÃO - '||TO_CHAR(VAR_DPROX_CARGA,'DD/MM/YYYY');
 	PR_GRAVA_MSG_LOG_CARGA(VAR_CROTNA, VAR_LOG, VAR_TPO_LOG_PROCESSO, NULL, NULL);
 	COMMIT;
	...
	FOR REG IN (SELECT PRD.ROWID
				...
				WHERE MRD.CCAMPA_DSTAQ				= PRD.CCAMPA_DSTAQ
				  ...
				  AND MRD.CCANAL_PROD_DW 			<> 8) LOOP --EXCLUI O CANAL FINASA POIS O MESMO NÃO TEM META PARA O RE
	...
	FOR REG IN (SELECT PRD.ROWID,
				...
				WHERE MRD.CCAMPA_DSTAQ				= PRD.CCAMPA_DSTAQ
				  ...
				  AND PRD.DAPURC_DSTAQ				= VAR_DPROX_CARGA 
				  AND PRD.CCAMPA_DSTAQ				= VAR_CCAMPA_DSTAQ
				  AND MRD.CCANAL_PROD_DW 			= 8) LOOP --SO O CANAL FINASA POIS O MESMO NÃO TEM META PARA O RE
	
		UPDATE POSIC_DSTAQ
		SET VPERC_CRSCT			=	REG.VPERC_CRSCT_AUTO
		  , VPERC_CRSCT_AUTO	=	REG.VPERC_CRSCT_AUTO
		  , DALT_REG			=	SYSDATE
		WHERE ROWID = REG.ROWID;
...
EXCEPTION                             
	WHEN VAR_FIM_PROCESSO_ERRO THEN
        ...
        VAR_LOG := 'ESTE PROCESSO FOI FINALIZADO COM ERRO (STATUS = 5). '||
