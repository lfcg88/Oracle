CREATE OR REPLACE PROCEDURE SGPB_PROC.SGPB6003_novo IS
-------------------------------------------------------------------------------------
...
BEGIN
	...
	FOR REG IN(SELECT CCANAL_DWXYZ 
 					, CCPF_CNPJ_BASE
 					, CTPO_PSSOA
 					, DAPURC_DSTAQ DEMIS
			        , sum(VPROD_RGNAL_AUTO) VPROD_AUTO
			        , sum(VPROD_REGNAL_RE) VPROD_RE
			   FROM POSIC_RGNAL_DSTAQ
			   GROUP BY CCANAL_DWXYZ
					  , CCPF_CNPJ_BASE
					  , CTPO_PSSOA
					  , DAPURC_DSTAQ)
	LOOP
      ...
      BEGIN
        ...
        from POSIC_CRRTR_DSTAQ
         where CCAMPA_DSTAQ        = VAR_CCAMPA_DSTAQ
           ...
           and CCANAL_DWXYZ		   = REG.CCANAL_DWXYZ		
...
END;
---------------------------------------------------------------------------------------------
PROCEDURE PR_PROD_CRRTR_MULT_CAMPA IS
BEGIN

	FOR REG_CAMP IN( SELECT CCAMPA_DSTAQ, DAPURC_DSTAQ, VPRMIO_DSTAQ
					 FROM CAMPA_DSTAQ 
	...
	LOOP
		VAR_LOG  := '*******************************************************************';
		PR_GRAVA_MSG_LOG_CARGA(VAR_CROTNA,  VAR_LOG,  VAR_LOG_PROCESSO, NULL, NULL);
		VAR_LOG  := 'INICIANDO PROCESSO DE APURAÇÃO DA PRODUÇÃO NACIONAL DO CORRETOR PARA A CAMPANHA: '||REG_CAMP.CCAMPA_DSTAQ;
		PR_GRAVA_MSG_LOG_CARGA(VAR_CROTNA,  VAR_LOG,  VAR_LOG_PROCESSO, NULL, NULL);
		
		VAR_LOG  := 'PRÊMIO DESTAQUE: ' || TO_CHAR(REG_CAMP.VPRMIO_DSTAQ) ;
		PR_GRAVA_MSG_LOG_CARGA(VAR_CROTNA,  VAR_LOG,  VAR_LOG_PROCESSO, NULL, NULL);
		
		VAR_LOG  := 'LIMPANDO POSSIVEIS ANTIGOS MOVIMENTOS DA DATA A SER PROCESSADA: '||TO_DATE(REG_CAMP.DAPURC_DSTAQ,'DD/MM/YYYY');
		PR_GRAVA_MSG_LOG_CARGA(VAR_CROTNA,  VAR_LOG,  VAR_LOG_PROCESSO, NULL, NULL);
		COMMIT;
		
		DELETE POSIC_CRRTR_DSTAQ
		WHERE DAPURC_DSTAQ = REG_CAMP.DAPURC_DSTAQ 
		  AND CCAMPA_DSTAQ = REG_CAMP.CCAMPA_DSTAQ;
...
BEGIN
	...
	PR_GRAVA_MSG_LOG_CARGA(VAR_CROTNA, VAR_LOG, VAR_LOG_PROCESSO, NULL, NULL);
 	...
 	VAR_LOG  := 'INICIANDO PROCESSO DE APURAÇÃO DA PRODUÇÃO NACIONAL DO CORRETOR(MULTICAMPANHA). ';
	PR_GRAVA_MSG_LOG_CARGA(VAR_CROTNA,  VAR_LOG,  VAR_LOG_PROCESSO, NULL, NULL);
	COMMIT;
	
 	PR_PROD_CRRTR_MULT_CAMPA;
