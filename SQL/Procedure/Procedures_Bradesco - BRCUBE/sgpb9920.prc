CREATE OR REPLACE PROCEDURE SGPB_PROC.SGPB9920 IS
  -------------------------------------------------------------------------------------------------
  ...
  VAR_ROTINA					VARCHAR2(10) := 'SGPB9920';
  ------------------------------------------------------------------------------------------------
...
BEGIN
   ...
   COMMIT;
             ...
             EXCEPTION
             WHEN OTHERS THEN
              var_log_erro := substr('erro no update da tabela parm_canal_vda_segur. ERRO: ' || SQLERRM,1,PC_UTIL_01.VAR_TAM_MSG_ERRO);
              ROLLBACK;
