CREATE OR REPLACE PROCEDURE SGPB_PROC.SGPB6707 IS
------------------------------------------------------------------------------------------------------------------------
...
VAR_CROTNA                 	ARQ_TRAB.CROTNA%TYPE        			:= 'SGPB6707'; 
VAR_CPARM_HIERQ_DSTAQ		PARM_HIERQ_DSTAQ.CPARM_HIERQ_DSTAQ%TYPE	:= 1; 
...
VAR_DINIC_CAMPA_DSTAQ		CAMPA_DSTAQ.DINIC_CAMPA_DSTAQ%TYPE;
...
BEGIN 
	...
	VAR_LOG := 'VAR_DFIM_META: '||VAR_DFIM_META
	||' VAR_DINIC_META: '||VAR_DINIC_META
	||' PI_CCAMPA: '||PI_CCAMPA;
	...
	FOR CUR_CRRTR_META IN (SELECT CDIR_RGNAL,CCPF_CNPJ_BASE,CTPO_PSSOA, CCANAL_PROD_DW, SUM(VPROD_AUTO) VPROD_AUTO, SUM(VPROD_RE) VPROD_RE
							FROM (
									...
									SELECT A.CDIR_RGNAL,D.CCPF_CNPJ_BASE,C.CTPO_PSSOA, 3 CCANAL_PROD_DW, SUM(A.VPRMIO_LIQ_AUTO) VPROD_AUTO, 0  VPROD_RE
										...
										GROUP BY A.CDIR_RGNAL,D.CCPF_CNPJ_BASE,C.CTPO_PSSOA
									UNION
									...
									SELECT A.CDIR_RGNAL,D.CCPF_CNPJ_BASE,C.CTPO_PSSOA, 3 CCANAL_PROD_DW, 0 VPROD_AUTO, sum(A.VPRMIO_EMTDO_CSSRO_CDIDO) VPROD_RE
							...
							WHERE CDIR_RGNAL IN (SELECT CRGNAL FROM GRP_RGNAL_DSTAQ WHERE CCAMPA_DSTAQ = PI_CCAMPA)			      
				 			GROUP BY CDIR_RGNAL,CCPF_CNPJ_BASE,CTPO_PSSOA,CCANAL_PROD_DW) LOOP
	--BEGIN --FOR
...
--------DADOS META
		...
		WROW_META_DSTAQ.DAPURC_DSTAQ			:= VAR_DINIC_CAMPA_DSTAQ;
		...
		BEGIN
			...
			VALUES (
				WROW_META_DSTAQ.CCAMPA_DSTAQ, WROW_META_DSTAQ.CHIERQ_PBLIC_ALVO_DSTAQ, VAR_DPROX_CARGA
...
BEGIN
	...
	FOR CUR_CAMPA IN (SELECT CCAMPA_DSTAQ,ICAMPA_DSTAQ,DINIC_CAMPA_DSTAQ
					  FROM CAMPA_DSTAQ
		...
		VAR_META_MIN_RE		:=	PC_PARAMETROS_DESTAQUE.FC_RCUPD_META_MIN_RE(CUR_CAMPA.CCAMPA_DSTAQ);
  		...
  		VAR_DINIC_CAMPA_DSTAQ	:= CUR_CAMPA.DINIC_CAMPA_DSTAQ;
