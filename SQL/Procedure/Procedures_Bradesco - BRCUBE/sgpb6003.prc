CREATE OR REPLACE PROCEDURE SGPB_PROC.SGPB6003 IS
-------------------------------------------------------------------------------------
...
--  OBJETIVO        : CORREÇÃO NA NOMENCLATURA
-------------------------------------------------------------------------------------
...
--VAR_VPROD_RE                POSIC_CRRTR_DSTAQ.VPROD_RE%TYPE                ; "nao existe  na tabela  POSIC_CRRTR_DSTAQ"
--VAR_VPROD_AUTO              POSIC_CRRTR_DSTAQ.VPROD_AUTO%TYPE              ;   "nao existe  na tabela  POSIC_CRRTR_DSTAQ"
--VAR_VPROD_RE                POSIC_DSTAQ.VPROD_RE%TYPE                ;    campo foi alterado pra esta tabela
--VAR_VPROD_AUTO              POSIC_DSTAQ.VPROD_AUTO%TYPE              ;  campo foi alterado pra esta tabela
...
--VAR_NRKING_PROD_NACIO       POSIC_CRRTR_DSTAQ.NRKING_PROD_NACIO%TYPE       ;  "nao existe  na tabela  POSIC_CRRTR_DSTAQ"
--VAR_NRKING_PERC_CRSCT_NACIO POSIC_CRRTR_DSTAQ.NRKING_PERC_CRSCT_NACIO%TYPE ;  "nao existe  na tabela  POSIC_CRRTR_DSTAQ"
--VAR_VPERC_CRSCT_NACIO       POSIC_CRRTR_DSTAQ.VPERC_CRSCT_NACIO%TYPE       ;  "nao existe  na tabela  POSIC_CRRTR_DSTAQ"
	  ...
	  BEGIN
          -- Cadastra Produção do Corretor na tabela POSIC_CRRTR_DSTAQ
             ...
             ( CCAMPA_DSTAQ,
		       ...
		       VPROD_PRMIO,
		       VPROD_PEND,
		      -- VPROD_RE,          "nao existe  na tabela  POSIC_CRRTR_DSTAQ"
		       --VPROD_AUTO,      "nao existe  na tabela  POSIC_CRRTR_DSTAQ"
		       CIDTFD_PRMIO_PCIAL,
		       --NRKING_PROD_NACIO,        "nao existe  na tabela  POSIC_CRRTR_DSTAQ"
		      -- NRKING_PERC_CRSCT_NACIO,  "nao existe  na tabela  POSIC_CRRTR_DSTAQ"
		       DINCL_REG,
		       DALT_REG)
		       ---VPERC_CRSCT_NACIO)    "nao existe  na tabela  POSIC_CRRTR_DSTAQ"
             VALUES
             ( PAR_CCAMPA_DSTAQ,
		       PAR_CTPO_PSSOA,
		       PAR_CCPF_CNPJ_BASE,
		      ...
		      -- nvl(PAR_NRKING_PERC_CRSCT_NACIO,0),
		       PAR_DINCL_REG,
		       NULL);            -- DALT_REG
		      -- 0);              -- VPERC_CRSCT_NACIO   "nao existe  na tabela  POSIC_CRRTR_DSTAQ"
          --
	  EXCEPTION
			    ...
			    FROM (
					    ...
					    select D.CCPF_CNPJ_BASE,
					      ...
					      WHERE B.CCAMPA_DSTAQ = VAR_CCAMPA_DSTAQ /*AND
					            (A.CCRRTR NOT BETWEEN B.CFAIXA_INIC_DSMTO_CRRTR) AND B.CFAIXA_FNAL_DSMTO_CRRTR)--"campo nao existe na tabela campa_dstaq*/ AND
					  ...
					  select D.CCPF_CNPJ_BASE,
					      ...
					      from VACPROD_CRRTR_CALC_BONUS_AT A,
					           ...
					           MPMTO_AG_CRRTR E
					     WHERE B.CCAMPA_DSTAQ = VAR_CCAMPA_DSTAQ /*AND
					            --(A.CCRRTR BETWEEN B.CFAIXA_INIC_DSMTO_CRRTR AND B.CFAIXA_FNAL_DSMTO_CRRTR) --"campo nao existe na tabela campa_dstaq */ AND
      ...
      BEGIN
        ...
        from POSIC_CRRTR_DSTAQ
	     ...
	     -- VAR_VPROD_AUTO           := VAR_VPROD_AUTO + REG.VPROD_AUTO;
	      --
	     /* if (0 + VAR_VPROD_RE) < 0 and VAR_CIND_DEB_CUPOM = 'S' then
    	...
    	WHEN NO_DATA_FOUND THEN
		   ...
		   --  VAR_QCUPOM_RETRD            := 0;
		    ...
		    -- VAR_NRKING_PROD_NACIO       := 0;
		     ...
		     --VAR_VPERC_CRSCT_NACIO       := 0;
		     EXCEPTION    --foi adiciona do o exception------
    	WHEN OTHERS THEN
    	    --
			VAR_CSIT_CTRLM := 6;
            --
