create or replace procedure sgpb_proc.SGPB6022 is
------------------------------------------------------------------------------------------------------------------------
...
--            OBS   : INCLUSÃO NOVO CRITÉRIO DE ATUALIZAÇÃO DIÁRIA DAS METAS DOS NOVOS CORRETORES 
...
--
VAR_CCAMPA_DSTAQ_01         NUMBER                            :=  1;
VAR_CCAMPA_DSTAQ_02         NUMBER                            :=  2;
--
...
--
--
PROCEDURE META_DIA_NOVO_CRRTR_CAMPA_01 
              (PAR_CCAMPA_DSTAQ      IN NUMBER,   
               PAR_DAPURC_DSTAQ      IN DATE, 
               PAR_DINIC_CAMPA_DSTAQ IN DATE, 
               PAR_DFIM_CAMPA_DSTAQ  IN DATE)
               --PAR_VMETA_MIN_AUTO    IN NUMBER, 
               --PAR_VMETA_MIN_RE      IN NUMBER)
  IS  -- POSICAO DIARIA PRODUÇÃO DA SUPEX
BEGIN
    --
   	VAR_LOG  := '------------------------------------------------------------------';
   	PR_GRAVA_MSG_LOG_CARGA(VAR_CROTNA, VAR_LOG, VAR_LOG_PROCESSO, NULL, NULL);
   	VAR_LOG  := 'INICIO APURAÇÃO DA META DOS CORRETORES NOVOS CAMPANHA 01';
   	PR_GRAVA_MSG_LOG_CARGA(VAR_CROTNA, VAR_LOG, VAR_LOG_PROCESSO, 3, NULL);
    VAR_LOG  := '-------------------------------------------------------------------';
   	PR_GRAVA_MSG_LOG_CARGA(VAR_CROTNA, VAR_LOG, VAR_LOG_PROCESSO, NULL, NULL);
   	--
	FOR REG IN(
				SELECT PRD.CRGNAL        CDIR_RGNAL,
				       PRD.CCPF_CNPJ_BASE,
				       PRD.CTPO_PSSOA
				  from POSIC_RGNAL_DSTAQ PRD
				    where PRD.DAPURC_DSTAQ      = VAR_DPROX_CARGA
				      AND PRD.CCAMPA_DSTAQ      = PAR_CCAMPA_DSTAQ
				      AND NOT EXISTS (SELECT 1
				                   ...
				                   WHERE CCAMPA_DSTAQ        = PAR_CCAMPA_DSTAQ
				                     ...
				                     AND ( TRUNC(DAPURC_DSTAQ) >= TRUNC(PAR_DINIC_CAMPA_DSTAQ) AND
				                           TRUNC(DAPURC_DSTAQ) <= TRUNC(PAR_DFIM_CAMPA_DSTAQ ))
				                          )
               )
    LOOP
   	--
...
END META_DIA_NOVO_CRRTR_CAMPA_01;

--
PROCEDURE PR_APURACAO_META_CRRTR IS
BEGIN
   --
   FOR REG IN(
			  SELECT CPD.CCAMPA_DSTAQ,    
			         CPD.DAPURC_DSTAQ,
			         CPD.DINIC_CAMPA_DSTAQ,
			         CPD.DFIM_CAMPA_DSTAQ
				     --CPD.VMETA_MIN_AUTO,  " CAMPO NAO EXISTE MAIS NA TABELA  CAMPA_DSTAQ "
				     --CPD.VMETA_MIN_RE   " CAMPO NAO EXISTE MAIS NA TABELA  CAMPA_DSTAQ "
			    FROM CAMPA_DSTAQ CPD
			   WHERE CPD.CIND_CAMPA_ATIVO  = VAR_CIND_CAMPA_ATIVO
			  ORDER BY CCAMPA_DSTAQ         
			  
             )
   LOOP
   --
     IF REG.CCAMPA_DSTAQ = VAR_CCAMPA_DSTAQ_01 THEN
        META_DIA_NOVO_CRRTR_CAMPA_01(REG.CCAMPA_DSTAQ,             -----PROCEDURE NAO ESTA MAIS NO BANCO---
                                     REG.DAPURC_DSTAQ, 
                                     REG.DINIC_CAMPA_DSTAQ, 
                                     REG.DFIM_CAMPA_DSTAQ );
                                     --REG.VMETA_MIN_AUTO,   " CAMPO NAO EXISTE MAIS NA TABELA  CAMPA_DSTAQ "
                                     --REG.VMETA_MIN_RE);    " CAMPO NAO EXISTE MAIS NA TABELA  CAMPA_DSTAQ "
     ELSIF REG.CCAMPA_DSTAQ = VAR_CCAMPA_DSTAQ_02 THEN              
        NULL; --META_DIA_NOVO_CRRTR_CAMPA_02;
     END IF;  
   --
   END LOOP;
  --
END PR_APURACAO_META_CRRTR;
--
