CREATE OR REPLACE PROCEDURE SGPB_PROC.SGPB0097(
...
-------------------------------------------------------------------------------------------------
  ...
  --      objetivo        : procedure retorna lista devalores para homologação banco/Finasa - Publico Selecionado
  --      alterações      :
  --                data  : -
  --                autor : -
  --                obs   : -
...
--
  JOIN mpmto_ag_crrtr mac
    ...
    AND TO_DATE(Intcompetfinal, 'YYYYMM') >= MAC.DENTRD_CRRTR_AG
    AND TO_DATE(Intcompetinicial, 'YYYYMM') < NVL(MAC.DSAIDA_CRRTR_AG, TO_DATE(99991231, 'YYYYMMDD'))
--
...
--
--
  join crrtr_eleit_campa cec on cec.ccpf_cnpj_base = cuc.ccpf_cnpj_base
