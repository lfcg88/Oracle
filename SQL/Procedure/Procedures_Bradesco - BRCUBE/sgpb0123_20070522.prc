CREATE OR REPLACE PROCEDURE SGPB_PROC.SGPB0123_20070522
(
    ...
    BEGIN
      ...
      LOOP
        ...
        --IF VAR_COUNT = 52 THEN
            ...
            -- As linhas abaixo foram retirada porque não vão rodar via dwscheduler.
            -- ass. wassily 11/04/2007
            --armazena no arquivo log a linha que deu erro
            --utl_file.put(VAR_LOG_ARQUIVO,VAR_REGISTRO_ARQUIVO);
            --nova linha
        ...
        EXCEPTION
          ...
          WHEN OTHERS THEN
            --
            VAR_LOG_ERRO:='Erro inserindo na carac_crrtr_canal.'||' -- LINHA: '||VAR_COUNT||' '||VAR_REGISTRO_ARQUIVO;
            -- As linhas abaixo foram retiradas. Pelo scheduler isso não roda.
            -- ass. Wassily - 11/04/2007
            --armazena no arquivo log a linha que deu erro
            --utl_file.put(VAR_LOG_ARQUIVO,VAR_REGISTRO_ARQUIVO);
