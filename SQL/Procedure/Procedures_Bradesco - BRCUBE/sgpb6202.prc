CREATE OR REPLACE PROCEDURE SGPB_PROC.SGPB6202(
...
intrREGIONAL       IN POSIC_RGNAL_DSTAQ.CRGNAL %TYPE
  ...
  --      OBJETIVO        : RANKING POR REGIONAL ( % ) 5 PRIMEIROS + O CORRETOR LOGADO
  --      ALTERAÇÕES      :
  --                DATA  :
  --                AUTOR :
  --                OBS   :
...
--           CIND_RGNAL_ALCAN_META
           ...
           END CASE
           --
     FROM (SELECT   PRD.NRKING_PERC_CRSCT_RGNAL,
                    PRD.CRGNAL,
                    PRD.CCPF_CNPJ_BASE,
                    CUC.IATUAL_CRRTR,
     ...
     WHERE ROWNUM <=5 --TOTAL DE LINHAS
    --  
    UNION
    --
    SELECT   'colocacao',
             PRD.NRKING_PERC_CRSCT_RGNAL,
             'regional',
             PRD.CRGNAL,
             'cpfCnpjBase',
             PRD.CCPF_CNPJ_BASE,
