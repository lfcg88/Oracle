CREATE OR REPLACE PROCEDURE SGPB_PROC.SGPB6217 IS
-------------------------------------------------------------------------------------------------
--      BRADESCO SEGUROS S.A.
--      DATA            : 24/02/2008
--      AUTOR           : FABIO GIGLIO - VALUE TEAM
--      PROGRAMA        : SGPB6217.SQL
--      OBJETIVO        : CARGA DAS TABELAS HIERQ_PBLIC_ALVO E META_DSTAQ PARA
--                        CANAL DE PRODUÇÃO 4 (BANCO) COM INFORMAÇÃO DE CAG_BCRIA E CBCO
--      ALTERAÇÕES      : 
--                DATA  : 
--                AUTOR : 
--                OBS   : 
...
VAR_CROTNA                 	ARQ_TRAB.CROTNA%TYPE        	    := 'SGPB6217';
--
...
/*---------------------------------------------------------------------------------------------*/
 BEGIN
   VAR_CGRP_RGNAL	 		:= TO_NUMBER(SUBSTR(VAR_REGISTRO_ARQUIVO, 11, 5));
 EXCEPTION
   WHEN OTHERS THEN
--
      VAR_CGRP_RGNAL		:= NULL;
      ...
      VAR_ERRO 	     	  	:= 'S';
--
      VAR_LOG := 'PROBLEMA AO CARREGAR CGRP_RGNAL DO ARQUIVO.'||
                 ' -- CCAMPA_DSTAQ: '||SUBSTR(VAR_REGISTRO_ARQUIVO, 1, 6)||
                 ' -- CPARM_HIERQ_DSTAQ: '||SUBSTR(VAR_REGISTRO_ARQUIVO, 7, 2)||
                 ' -- CCANAL_PROD_DW: '||SUBSTR(VAR_REGISTRO_ARQUIVO, 9, 2)||
                 ' -- CGRP_RGNAL: '||SUBSTR(VAR_REGISTRO_ARQUIVO, 11,5)||
                 ' -- ERRO ORACLE: '||SUBSTR(SQLERRM, 1, 120);
      PR_GRAVA_MSG_LOG_CARGA(VAR_CROTNA, VAR_LOG, VAR_LOG_DADO, NULL, NULL);
      --DBMS_OUTPUT.PUT_LINE(SUBSTR(VAR_LOG,1,255));  
--
...
/*---------------------------------------------------------------------------------------------*/
 BEGIN
   VAR_CBCO 				:= TO_NUMBER(SUBSTR(VAR_REGISTRO_ARQUIVO, 16, 4));
 EXCEPTION
   WHEN OTHERS THEN
--
      VAR_CBCO				:= NULL;
      ...
      VAR_ERRO 	     	  	:= 'S';
--
      VAR_LOG := 'PROBLEMA AO CARREGAR CBCO DO ARQUIVO.'||
                 ' -- CCAMPA_DSTAQ: '||SUBSTR(VAR_REGISTRO_ARQUIVO, 1, 6)||
                 ' -- CPARM_HIERQ_DSTAQ: '||SUBSTR(VAR_REGISTRO_ARQUIVO, 7, 2)||
                 ' -- CCANAL_PROD_DW: '||SUBSTR(VAR_REGISTRO_ARQUIVO, 9, 2)||
                 ' -- CBCO: '||SUBSTR(VAR_REGISTRO_ARQUIVO, 16, 4)||
                 ' -- ERRO ORACLE: '||SUBSTR(SQLERRM, 1, 120);
      PR_GRAVA_MSG_LOG_CARGA(VAR_CROTNA, VAR_LOG, VAR_LOG_DADO, NULL, NULL);
      --DBMS_OUTPUT.PUT_LINE(SUBSTR(VAR_LOG,1,255));  
--
...
/*---------------------------------------------------------------------------------------------*/
 BEGIN
   VAR_CAG_BCRIA			:= TO_NUMBER(SUBSTR(VAR_REGISTRO_ARQUIVO, 20, 4));
 EXCEPTION
   WHEN OTHERS THEN
--
      VAR_CAG_BCRIA			:= NULL;
      ...
      VAR_ERRO 	     	  	:= 'S';
--
      VAR_LOG := 'PROBLEMA AO CARREGAR CAG_BCRIA DO ARQUIVO.'||
                 ' -- CCAMPA_DSTAQ: '||SUBSTR(VAR_REGISTRO_ARQUIVO, 1, 6)||
                 ' -- CPARM_HIERQ_DSTAQ: '||SUBSTR(VAR_REGISTRO_ARQUIVO, 7, 2)||
                 ' -- CCANAL_PROD_DW: '||SUBSTR(VAR_REGISTRO_ARQUIVO, 9, 2)||
                 ' -- CAG_BCRIA: '||SUBSTR(VAR_REGISTRO_ARQUIVO, 20, 4)||
                 ' -- ERRO ORACLE: '||SUBSTR(SQLERRM, 1, 120);
      PR_GRAVA_MSG_LOG_CARGA(VAR_CROTNA, VAR_LOG, VAR_LOG_DADO, NULL, NULL);
      --DBMS_OUTPUT.PUT_LINE(SUBSTR(VAR_LOG,1,255));  
--
...
/*---------------------------------------------------------------------------------------------*/
 BEGIN
   WROW.VMETA_AUTO		:= TO_NUMBER(SUBSTR(VAR_REGISTRO_ARQUIVO, 24, 15))/100;
 EXCEPTION
   WHEN OTHERS THEN
--
...
--
      VAR_LOG := 'PROBLEMA AO CARREGAR VMETA_AUTO DO ARQUIVO.'||
                 ...
                 ' -- CPARM_HIERQ_DSTAQ: '||SUBSTR(VAR_REGISTRO_ARQUIVO, 7, 2)||
                 ' -- CCANAL_PROD_DW: '||SUBSTR(VAR_REGISTRO_ARQUIVO, 9, 2)||
                 ' -- VMETA_AUTO: '||SUBSTR(VAR_REGISTRO_ARQUIVO, 24, 15)||
                 ' -- ERRO ORACLE: '||SUBSTR(SQLERRM, 1, 120);
      PR_GRAVA_MSG_LOG_CARGA(VAR_CROTNA, VAR_LOG, VAR_LOG_DADO, NULL, NULL);
      --DBMS_OUTPUT.PUT_LINE(SUBSTR(VAR_LOG,1,255));  
--   
...
/*---------------------------------------------------------------------------------------------*/
 BEGIN
   WROW.VMETA_RE			:= TO_NUMBER(SUBSTR(VAR_REGISTRO_ARQUIVO, 39, 15))/100;
 EXCEPTION
   WHEN OTHERS THEN
--
      WROW.VMETA_RE			:= NULL;
...
--
      VAR_LOG := 'PROBLEMA AO CARREGAR VMETA_RE DO ARQUIVO.'||
                 ...
                 ' -- CPARM_HIERQ_DSTAQ: '||SUBSTR(VAR_REGISTRO_ARQUIVO, 7, 2)||
                 ' -- CCANAL_PROD_DW: '||SUBSTR(VAR_REGISTRO_ARQUIVO, 9, 2)||
                 ' -- VMETA_RE: '||SUBSTR(VAR_REGISTRO_ARQUIVO, 39, 15)||
                 ' -- ERRO ORACLE: '||SUBSTR(SQLERRM, 1, 120);
      PR_GRAVA_MSG_LOG_CARGA(VAR_CROTNA, VAR_LOG, VAR_LOG_DADO, NULL, NULL);
      --DBMS_OUTPUT.PUT_LINE(SUBSTR(VAR_LOG,1,255));  
--   
...
--              
/*---------------------------------------------------------------------------------------------*/
-- CANAL DE PRODUÇÃO 4 (BANCO) COM INFORMAÇÃO DE CAG_BCRIA E CBCO OU CANAL DE PRODUÇÃO 8 (FINASA)
  IF VAR_ERRO = 'N' THEN
     IF WROW.CCANAL_PROD_DW = 4 THEN
...
--
        IF WROW.CHIERQ_PBLIC_ALVO_DSTAQ IS NULL THEN 
	       VAR_LOG  := 'CHIERQ_PBLIC_ALVO_DSTAQ NÃO RECUPERADO.'||
                       ...
                       ' -- CGRP_RGNAL: '||TO_CHAR(VAR_CGRP_RGNAL)||
 ...
 EXCEPTION
    WHEN OTHERS THEN 
         VAR_LOG := 'ERRO NO INSERT DOS DADOS DA TABELA ' || VAR_TABELA_P ||
                    ' -- LINHA DO REGISTRO: '|| SUBSTR(VAR_REGISTRO_ARQUIVO, 1, 53)||      	   
                    ' -- ERRO ORACLE: ' || SUBSTR(SQLERRM, 1, 120);
	     PR_GRAVA_MSG_LOG_CARGA(VAR_CROTNA, VAR_LOG, VAR_LOG_PROCESSO, NULL, NULL);
         --DBMS_OUTPUT.PUT_LINE(SUBSTR(VAR_LOG,1,255));
--
...
--   
        VAR_CSIT_CTRLM  := 6;
--   
        VAR_LOG := 'ERRO NO TRATA_BODY. LINHA DO REGISTRO: '||SUBSTR( VAR_REGISTRO_ARQUIVO, 1, 53)||
                   ' -- ERRO ORACLE: '|| SUBSTR(SQLERRM, 1, 120);
	    PR_GRAVA_MSG_LOG_CARGA(VAR_CROTNA, VAR_LOG, VAR_LOG_PROCESSO, NULL, NULL);
	    --DBMS_OUTPUT.PUT_LINE(SUBSTR(VAR_LOG,1,255));
--    
