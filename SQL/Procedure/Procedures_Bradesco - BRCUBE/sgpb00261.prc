CREATE OR REPLACE PROCEDURE SGPB_PROC.Sgpb00261
(
				 ...
				 ( --
					SELECT Apc.Ccanal_Vda_Segur,
					 ...
					 WHERE Apc.Ccompt_Apurc BETWEEN Intcomptemp AND Intrcompetencia
								 ...
								 AND
								 Apc.Csit_Apurc IN ('AP', 'BG', 'LM', 'LG', 'PR', 'PL', 'LP')
								 AND NOT EXISTS
					 (SELECT 1
	 ...
	 WHERE Cind_Apurc_Selec = 1
				 ...
				 ( --
							SELECT Tap.Ccrrtr,
							 ...
							 GROUP BY Tap.Ccrrtr,
												 Tap.Cund_Prod
							HAVING COUNT(Ccrrtr) < Intqmes_Perdc_Pgto --
							);
	COMMIT;
	--
	--
							...
							SELECT Apc.Ccanal_Vda_Segur,
							 ...
							 WHERE Apc.Ccompt_Apurc < Intcomptemp
										...
										--             AND APC.CTPO_APURC = TIPO
										 AND Apc.Ccanal_Vda_Segur = Pc_Util_01.Banco
										 AND NOT EXISTS
							 (SELECT 1
												FROM Papel_Apurc_Pgto Pap
											 WHERE Pap.Ccanal_Vda_Segur = Apc.Ccanal_Vda_Segur
	...
	OPEN c_Apuracoes FOR
		...
		--
			JOIN ( --
						SELECT c.Ccpf_Cnpj_Base,
										c.Ctpo_Pssoa,
										CASE
											WHEN SUM(Pc.Vprod_Crrtr * (Apc.Pbonus_Apurc / 100)) >=
													 Intvmin_Librc_Pgto THEN
											 'TRUE'
											ELSE
											 'FALSE'
						...
						--
							...
							LEFT JOIN Papel_Apurc_Pgto Pap ON Pap.Ccanal_Vda_Segur =
																								...
																								AND Pap.Ctpo_Apurc =
																								Apc.Ctpo_Apurc
																								AND Pap.Ccompt_Apurc =
																								Apc.Ccompt_Apurc
																								AND Pap.Cgrp_Ramo_Plano =
																								Apc.Cgrp_Ramo_Plano
																								AND Pap.Ccompt_Prod =
																								Apc.Ccompt_Prod
																								AND Pap.Ctpo_Comis =
		...
		--
			LEFT JOIN Papel_Apurc_Pgto Pap ON Pap.Ccanal_Vda_Segur =
																				...
																				AND Pap.Ctpo_Apurc = Apc.Ctpo_Apurc
																				AND
																				Pap.Ccompt_Apurc = Apc.Ccompt_Apurc
																				AND Pap.Cgrp_Ramo_Plano =
																				Apc.Cgrp_Ramo_Plano
																				AND
																				Pap.Ccompt_Prod = Apc.Ccompt_Prod
																				AND Pap.Ctpo_Comis = Apc.Ctpo_Comis
																				AND Pap.Ccrrtr = Apc.Ccrrtr
																				AND Pap.Cund_Prod = Apc.Cund_Prod
																				AND Pap.Cindcd_Papel = 0
...
EXCEPTION
	WHEN OTHERS THEN
		...
		Var_Log_Erro := Substr('COD.ERRO: ' || Chrlocalerro || ' COMPET: ' ||
													 To_Char(Intrcompetencia) || ' CANAL: ' ||
													 To_Char(Intcodcanal_Vda_Segur) || ' # ' ||
													 SQLERRM,
													 1,
													 Pc_Util_01.Var_Tam_Msg_Erro);
		--
		--    DBMS_OUTPUT.PUT_LINE(VAR_LOG_ERRO);
		PR_GRAVA_MSG_LOG_CARGA('SGPB00261',
																Var_Log_Erro,
		...
		--
		--
		PR_ATUALIZA_STATUS_ROTINA('SGPB00261',
																	 708,
