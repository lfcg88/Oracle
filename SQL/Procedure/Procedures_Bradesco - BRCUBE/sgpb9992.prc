CREATE OR REPLACE PROCEDURE SGPB_PROC.SGPB9992
IS
  -------------------------------------------------------------------------------------------------
  ...
  --      OBJETIVO        : Corrigir Corretores Desmembrados (apenas faixa 800, alguns casos, DWRE)
  --      ALTERAÇÕES      :
  -------------------------------------------------------------------------------------------------
  ...
  VAR_CROTNA 					CONSTANT CHAR(8) := 'SGPB9992';
  VAR_IROTNA 					CONSTANT INT := 722;
  ABRE_ARQUIVO_EXCEPTION 		EXCEPTION;
  ERRO_NA_LEITURA				EXCEPTION;
  VAR_DCARGA                 	date;
  VAR_DPROX_CARGA            	date;
  VAR_IDTRIO_TRAB            	varchar2(100) := '/x0205/P002/dwscheduler/scripts';  -- '/x0305/D001/sgpb/arquivo'; 
  VAR_IARQ_TRAB              	varchar2(100) := 'SGPB_AJUSTE_DWRE.dat'; -- 'SGPB_AJUSTE_ULTIMO.dat';
    ...
    EXCEPTION
        WHEN OTHERS THEN
             VAR_LOG_ERRO := 'ERRO NA CONVERSAO DA DATA DE EMISSAO DA APOLICE PARA DATA DE ENTRADA. LINHA: '||VAR_COUNT||
                             ...
                             ' V_APOLC: '||V_APOLC||' ERRO: '||SUBSTR(SQLERRM,1,200);
    ...
    EXCEPTION            
        ...
        WHEN OTHERS THEN
             VAR_LOG_ERRO := 'ERRO NO ACESSO AO DESMEMBRAMENTO. LINHA: '||VAR_COUNT||
                             ' V_CCRRTR_ORIGN: '||V_CCRRTR_ORIGN||
                             ' V_CUND_PROD: '||V_CUND_PROD||' V_EMISSAO_APOLC:'||VAR_DEMISSAO_C||' V_CIA: '||V_CIA||
                             ' V_APOLC: '||V_APOLC||' V_VALOR_PREMIO: '||V_VALOR_PREMIO||' ERRO: '||SUBSTR(SQLERRM,1,200);
             PR_GRAVA_MSG_LOG_CARGA(VAR_CROTNA,VAR_LOG_ERRO,'P',NULL,NULL);
             PR_ATUALIZA_STATUS_ROTINA(VAR_CROTNA,VAR_IROTNA,PC_UTIL_01.VAR_ROTNA_PE);
             COMMIT;      
             Raise_Application_Error(-20008,VAR_LOG_ERRO); 
 	...
 	BEGIN
 	   SELECT CCRRTR, min(CITEM_APOLC) into V_CRRTR_APOLC, V_MINIMO_ITEM from APOLC_PROD_CRRTR 
 	   			where cund_prod         = V_CUND_PROD               and
 	   			      ...
 	   			      capolc            = V_APOLC                   and
 	   			      trunc(demis_apolc)= trunc(VAR_DEMISSAO_APOLC) and
 	   			      CRAMO_APOLC not in (120,460,519)
 	   			      GROUP BY CCRRTR;
 	END;
                         ...
                         UPDATE APOLC_PROD_CRRTR SET
                                CCRRTR = V_CCRRTR_DSMEM 
                         		where 	cund_prod         = V_CUND_PROD               and
 	   			      					...
 	   			      					capolc            = V_APOLC                   and
 	   			      					trunc(demis_apolc)= trunc(VAR_DEMISSAO_APOLC) and
 	   			      					CRAMO_APOLC not in (120,460,519)			  and
 ...
 EXCEPTION
 	WHEN NO_DATA_FOUND THEN
 	     VAR_LOG_ERRO := 'APOLICE NÃO ENCONTRADA NO SGPB. LINHA: '||VAR_COUNT||
                    ...
                    ' V_EMISSAO_APOLC:'||VAR_DEMISSAO_C||' V_CIA: '||V_CIA||
                    ' V_APOLC: '||V_APOLC;
         PR_GRAVA_MSG_LOG_CARGA(VAR_CROTNA,VAR_LOG_ERRO,'P',NULL,NULL);
         COMMIT;    
    WHEN OTHERS THEN
         ...
         PR_GRAVA_MSG_LOG_CARGA(VAR_CROTNA,VAR_LOG_ERRO,'P',NULL,NULL);
         COMMIT;
  -- o IF ESTAVA AQUI, ELE NÃO IRIA SER EXECUTADO.........................
END DETALHE;
  --
BEGIN
   ...
   -- Abrindo o Arquivo
   BEGIN
           VAR_ARQUIVO := UTL_FILE.FOPEN('/x0205/P002/dwscheduler/scripts','SGPB_AJUSTE_DWRE.dat','R');            
