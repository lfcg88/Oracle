CREATE OR REPLACE PROCEDURE SGPB_PROC.SGPB6751 IS
------------------------------------------------------------------------------------------------------------------------
...
VAR_CROTNA                 	ARQ_TRAB.CROTNA%TYPE        	:= 'SGPB6751';
...
CURSOR CUR_PRO_CANAL_EXTRA_BCO IS
	...
	FROM (
		...
		WHERE B.CCAMPA_DSTAQ = VAR_CCAMPA_DSTAQ AND A.CCANAL_PROD_DW IN (3,5)
			...
			AND X.CRGNAL = A.CDIR_RGNAL AND X.CCAMPA_DSTAQ = B.CCAMPA_DSTAQ AND A.DEMIS_ENDSS = VAR_DPROX_CARGA
		...
		WHERE B.CCAMPA_DSTAQ = VAR_CCAMPA_DSTAQ AND A.CCANAL_PROD_DW IN (3,5)
			AND A.CCRRTR = C.CCRRTR AND A.CSUCUR = C.CUND_PROD AND C.CCPF_CNPJ_BASE = D.CCPF_CNPJ_BASE
			AND E.CCAMPA_DSTAQ = B.CCAMPA_DSTAQ AND A.CRAMO = E.CRAMO AND X.CRGNAL = A.CDIR_RGNAL AND X.CCAMPA_DSTAQ = B.CCAMPA_DSTAQ AND A.DEMIS_PRMIO = VAR_DPROX_CARGA
			AND NOT EXISTS (SELECT CCONTD_PARM_CARGA FROM CAMPA_PARM_CARGA_DSTAQ X
					WHERE X.CCAMPA_DSTAQ = B.CCAMPA_DSTAQ AND X.CPARM_CARGA_DSTAQ = 2 AND A.CSUCUR = CCONTD_PARM_CARGA)
			AND NOT EXISTS (SELECT CCONTD_PARM_CARGA FROM CAMPA_PARM_CARGA_DSTAQ X
					WHERE X.CCAMPA_DSTAQ = B.CCAMPA_DSTAQ AND X.CPARM_CARGA_DSTAQ = 3 AND A.CCRRTR   = CCONTD_PARM_CARGA )
		...
		WHERE B.CCAMPA_DSTAQ = VAR_CCAMPA_DSTAQ AND A.CCANAL_PROD_DW IN (3,5)
			...
			AND A.CRAMO = F.CRAMO AND X.CRGNAL = A.CDIR_RGNAL AND X.CCAMPA_DSTAQ = B.CCAMPA_DSTAQ AND A.DEMIS_PRMIO  = VAR_DPROX_CARGA
			AND NOT EXISTS (SELECT CCONTD_PARM_CARGA FROM CAMPA_PARM_CARGA_DSTAQ X
					WHERE X.CCAMPA_DSTAQ = B.CCAMPA_DSTAQ AND X.CPARM_CARGA_DSTAQ = 2 AND A.CSUCUR = CCONTD_PARM_CARGA)
			AND NOT EXISTS (SELECT CCONTD_PARM_CARGA FROM CAMPA_PARM_CARGA_DSTAQ X
					WHERE X.CCAMPA_DSTAQ = B.CCAMPA_DSTAQ AND X.CPARM_CARGA_DSTAQ = 3 AND A.CCRRTR = CCONTD_PARM_CARGA )
	 ...
	 WHERE CRGNAL IN (SELECT CRGNAL FROM GRP_RGNAL_DSTAQ WHERE CCAMPA_DSTAQ = VAR_CCAMPA_DSTAQ)
	 GROUP BY CRGNAL
		, CGRP_RGNAL
		, CCPF_CNPJ_BASE
		, CTPO_PSSOA
...
CURSOR CUR_PRO_CANAL_BCO_AG IS
	...
	FROM (
		-- PRODUCAO AUTO QUE NÃO TEM DESMEMBRAMENTO
		SELECT A.CDIR_RGNAL CRGNAL,NULL CGRP_RGNAL, NULL CCPF_CNPJ_BASE, NULL CTPO_PSSOA, A.CCANAL_PROD_DW , A.CCHAVE_AG_BCRIA CAG_BCRIA, NULL CGRP_FNASA, A.CBCO
			, SUM(A.VPRMIO_LIQ_AUTO) VPROD_AUTO, 0  VPROD_RE
		...
		WHERE B.CCAMPA_DSTAQ = VAR_CCAMPA_DSTAQ AND A.CCANAL_PROD_DW IN (4)
			AND A.CCRRTR = C.CCRRTR AND A.CSUCUR = C.CUND_PROD AND C.CCPF_CNPJ_BASE = D.CCPF_CNPJ_BASE
			AND A.DEMIS_ENDSS = VAR_DPROX_CARGA  AND NOT (A.CCRRTR BETWEEN 880005 AND 889998)
			AND NOT EXISTS (SELECT CCONTD_PARM_CARGA FROM CAMPA_PARM_CARGA_DSTAQ X
					WHERE X.CCAMPA_DSTAQ = B.CCAMPA_DSTAQ AND X.CPARM_CARGA_DSTAQ = 2 AND A.CSUCUR = CCONTD_PARM_CARGA)
			AND NOT EXISTS (SELECT CCONTD_PARM_CARGA FROM CAMPA_PARM_CARGA_DSTAQ X
					WHERE X.CCAMPA_DSTAQ = B.CCAMPA_DSTAQ AND X.CPARM_CARGA_DSTAQ = 3 AND A.CCRRTR   = CCONTD_PARM_CARGA )
		GROUP BY A.CDIR_RGNAL,A.CCANAL_PROD_DW, A.CCHAVE_AG_BCRIA, A.CBCO
		UNION
		-- PRODUCAO AUTO QUE COM DESMEMBRAMENTO
		SELECT A.CDIR_RGNAL CRGNAL,NULL CGRP_RGNAL, NULL CCPF_CNPJ_BASE, NULL CTPO_PSSOA, A.CCANAL_PROD_DW , A.CCHAVE_AG_BCRIA CAG_BCRIA, NULL CGRP_FNASA, A.CBCO
			, SUM(A.VPRMIO_LIQ_AUTO) VPROD_AUTO, 0  VPROD_RE
		...
		WHERE B.CCAMPA_DSTAQ = VAR_CCAMPA_DSTAQ AND A.CCANAL_PROD_DW IN (4)
			AND A.CCRRTR = C.CCRRTR AND A.CSUCUR = C.CUND_PROD AND C.CCPF_CNPJ_BASE = D.CCPF_CNPJ_BASE
			AND E.CCAMPA_DSTAQ = B.CCAMPA_DSTAQ AND A.CRAMO = E.CRAMO AND A.DEMIS_PRMIO = VAR_DPROX_CARGA
			AND NOT (A.CCRRTR BETWEEN 880005 AND 889998)
			AND NOT EXISTS (SELECT CCONTD_PARM_CARGA FROM CAMPA_PARM_CARGA_DSTAQ X
					WHERE X.CCAMPA_DSTAQ = B.CCAMPA_DSTAQ AND X.CPARM_CARGA_DSTAQ = 2 AND A.CSUCUR = CCONTD_PARM_CARGA)
			AND NOT EXISTS (SELECT CCONTD_PARM_CARGA FROM CAMPA_PARM_CARGA_DSTAQ X
					WHERE X.CCAMPA_DSTAQ = B.CCAMPA_DSTAQ AND X.CPARM_CARGA_DSTAQ = 3 AND A.CCRRTR   = CCONTD_PARM_CARGA )
		...
		WHERE B.CCAMPA_DSTAQ = VAR_CCAMPA_DSTAQ AND A.CCANAL_PROD_DW IN (4)
			...
			AND A.DEMIS_PRMIO < NVL(E.DSAIDA_CRRTR_AG, TO_DATE(99991231, 'YYYYMMDD')) AND E.CCRRTR_DSMEM = C.CCRRTR
			AND E.CUND_PROD = C.CUND_PROD AND C.CCPF_CNPJ_BASE = D.CCPF_CNPJ_BASE AND F.CCAMPA_DSTAQ = B.CCAMPA_DSTAQ
			AND A.CRAMO = F.CRAMO AND A.DEMIS_PRMIO  = VAR_DPROX_CARGA
			AND NOT (A.CCRRTR BETWEEN 880005 AND 889998)
			AND NOT EXISTS (SELECT CCONTD_PARM_CARGA FROM CAMPA_PARM_CARGA_DSTAQ X
					WHERE X.CCAMPA_DSTAQ = B.CCAMPA_DSTAQ AND X.CPARM_CARGA_DSTAQ = 2 AND A.CSUCUR = CCONTD_PARM_CARGA)
			AND NOT EXISTS (SELECT CCONTD_PARM_CARGA FROM CAMPA_PARM_CARGA_DSTAQ X
...
CURSOR CUR_PRO_CANAL_BCO_CNPJ IS
	...
	FROM (
		-- PRODUCAO AUTO QUE NÃO TEM DESMEMBRAMENTO
		SELECT A.CDIR_RGNAL CRGNAL,NULL CGRP_RGNAL,D.CCPF_CNPJ_BASE,C.CTPO_PSSOA, A.CCANAL_PROD_DW , NULL CAG_BCRIA, NULL CGRP_FNASA
			, SUM(A.VPRMIO_LIQ_AUTO) VPROD_AUTO, 0  VPROD_RE
		FROM VACPROD_CRRTR_CALC_BONUS_AT A,	CAMPA_DSTAQ B, CRRTR C,CRRTR_UNFCA_CNPJ D, GRP_RGNAL_DSTAQ X
		WHERE B.CCAMPA_DSTAQ = VAR_CCAMPA_DSTAQ AND A.CCANAL_PROD_DW IN (4)
			AND A.CCRRTR = C.CCRRTR AND A.CSUCUR = C.CUND_PROD AND C.CCPF_CNPJ_BASE = D.CCPF_CNPJ_BASE
...
CURSOR CUR_PRO_CANAL_FNASA IS
	...
	FROM (
		...
		SELECT NULL CRGNAL,NULL CGRP_RGNAL,D.CCPF_CNPJ_BASE,C.CTPO_PSSOA, A.CCANAL_PROD_DW , NULL CAG_BCRIA, NULL CGRP_FNASA
			, SUM(A.VPRMIO_LIQ_AUTO) VPROD_AUTO, 0  VPROD_RE
		FROM VACPROD_CRRTR_CALC_BONUS_AT A,	CAMPA_DSTAQ B, cpvo.CRRTR C,cpvo.CRRTR_UNFCA_CNPJ D
		WHERE B.CCAMPA_DSTAQ = VAR_CCAMPA_DSTAQ AND A.CCANAL_PROD_DW IN (8)
			AND A.CCRRTR = C.CCRRTR AND A.CSUCUR = C.CUND_PROD AND C.CCPF_CNPJ_BASE = D.CCPF_CNPJ_BASE
			AND A.DEMIS_ENDSS = VAR_DPROX_CARGA
			AND NOT EXISTS (SELECT CCONTD_PARM_CARGA FROM CAMPA_PARM_CARGA_DSTAQ X
					WHERE X.CCAMPA_DSTAQ = B.CCAMPA_DSTAQ AND X.CPARM_CARGA_DSTAQ = 2 AND A.CSUCUR = CCONTD_PARM_CARGA)
			AND NOT EXISTS (SELECT CCONTD_PARM_CARGA FROM CAMPA_PARM_CARGA_DSTAQ X
					WHERE X.CCAMPA_DSTAQ = B.CCAMPA_DSTAQ AND X.CPARM_CARGA_DSTAQ = 3 AND A.CCRRTR   = CCONTD_PARM_CARGA )
		...
		SELECT NULL CRGNAL,NULL CGRP_RGNAL,D.CCPF_CNPJ_BASE,C.CTPO_PSSOA, A.CCANAL_PROD_DW , NULL CAG_BCRIA, NULL CGRP_FNASA
			, SUM(A.VPRMIO_LIQ_AUTO) VPROD_AUTO, 0  VPROD_RE
		FROM VACPROD_CRRTR_CALC_BONUS_AT A,CAMPA_DSTAQ B,cpvo.CRRTR C,cpvo.CRRTR_UNFCA_CNPJ D,MPMTO_AG_CRRTR E --, GRP_RGNAL_DSTAQ X
		WHERE B.CCAMPA_DSTAQ = VAR_CCAMPA_DSTAQ AND A.CCANAL_PROD_DW IN (8)
			AND A.CCRRTR = E.CCRRTR_ORIGN AND A.CSUCUR = E.CUND_PROD AND A.DEMIS_ENDSS >= E.DENTRD_CRRTR_AG
			AND A.DEMIS_ENDSS < NVL(E.DSAIDA_CRRTR_AG, TO_DATE(99991231, 'YYYYMMDD'))AND E.CCRRTR_DSMEM = C.CCRRTR AND E.CUND_PROD = C.CUND_PROD AND C.CCPF_CNPJ_BASE = D.CCPF_CNPJ_BASE
			AND A.DEMIS_ENDSS  = VAR_DPROX_CARGA
	 ...
	 WHERE CCPF_CNPJ_BASE IN (SELECT CCPF_CNPJ_BASE FROM GRP_FNASA_DSTAQ WHERE CCAMPA_DSTAQ = VAR_CCAMPA_DSTAQ)
	 GROUP BY CRGNAL
		, CGRP_RGNAL
		, CCPF_CNPJ_BASE
		, CTPO_PSSOA
...
EXCEPTION
	WHEN VAR_FIM_PROCESSO_ERRO THEN
		VAR_CSIT_CTRLM		:= 5;
		VAR_LOG := 'PROBLEMA NO SUBPROGRAMA APAGA_MOVIMENTO_DIA_CANAL. '
...
---------------------------------------------------------------------------------------------------------------------------
PROCEDURE INSERE_POSIC_DSTAQ IS
BEGIN
...
EXCEPTION
	WHEN OTHERS THEN
		...
		PR_GRAVA_MSG_LOG_CARGA(VAR_CROTNA, VAR_LOG, VAR_LOG_ADVERTENCIA, NULL, NULL);
		RAISE VAR_FIM_PROCESSO_ERRO;
END INSERE_POSIC_DSTAQ;
---------------------------------------------------------------------------------------------------------------------------
PROCEDURE PR_POSIC_DIA_PROD_COMPL (	 PI_CCAMPA CAMPA_DSTAQ.CCAMPA_DSTAQ%TYPE
									,PI_DAPURC_DSTAQ CAMPA_DSTAQ.DAPURC_DSTAQ%TYPE
									,PI_CCANAL_PROD_DW POSIC_DSTAQ.CCANAL_PROD_DW%TYPE
									,PI_CCPARM_HIERQ_DSTAQ POSIC_DSTAQ.CPARM_HIERQ_DSTAQ%TYPE)IS  -- POSICAO DIARIA PRODUÇÃO SUPEX COMPLEMENTAR
CURSOR CUR_PROD_COMPL IS
SELECT	 CCAMPA_DSTAQ
	...
	FROM POSIC_DSTAQ
	WHERE CCAMPA_DSTAQ = PI_CCAMPA
	  AND TRUNC(DAPURC_DSTAQ) = PI_DAPURC_DSTAQ
	  AND CCANAL_PROD_DW = PI_CCANAL_PROD_DW
	  AND CPARM_HIERQ_DSTAQ = PI_CCPARM_HIERQ_DSTAQ
	  AND CHIERQ_PBLIC_ALVO_DSTAQ NOT IN (
...
VAR_CCUR_PROD_COMPL	CUR_PROD_COMPL%ROWTYPE;
BEGIN

	VAR_LOG  := 'INICIO EXECUÇÃO CURSOR DE RECUPERAÇÃO DA PRODUÇÃO COMPLEMENTAR CANAL: '||
				PI_CCANAL_PROD_DW||' HIERARQUIA: '||PI_CCPARM_HIERQ_DSTAQ;
   	PR_GRAVA_MSG_LOG_CARGA(VAR_CROTNA, VAR_LOG, VAR_LOG_PROCESSO, NULL, NULL);

    OPEN CUR_PROD_COMPL;

	FETCH  CUR_PROD_COMPL INTO VAR_CCUR_PROD_COMPL;

	WHILE CUR_PROD_COMPL%FOUND LOOP
    
		WROW_POSIC_DSTAQ							:= NULL;
    
		WROW_POSIC_DSTAQ.CCAMPA_DSTAQ				:= VAR_CCUR_PROD_COMPL.CCAMPA_DSTAQ;
		WROW_POSIC_DSTAQ.CHIERQ_PBLIC_ALVO_DSTAQ	:= VAR_CCUR_PROD_COMPL.CHIERQ_PBLIC_ALVO_DSTAQ;
		WROW_POSIC_DSTAQ.DAPURC_DSTAQ				:= VAR_DPROX_CARGA;
		WROW_POSIC_DSTAQ.CPARM_HIERQ_DSTAQ			:= VAR_CCUR_PROD_COMPL.CPARM_HIERQ_DSTAQ;
		WROW_POSIC_DSTAQ.VPROD_AUTO					:= VAR_CCUR_PROD_COMPL.VPROD_AUTO;
		WROW_POSIC_DSTAQ.VPROD_RE					:= VAR_CCUR_PROD_COMPL.VPROD_RE;
		WROW_POSIC_DSTAQ.CCANAL_PROD_DW				:= VAR_CCUR_PROD_COMPL.CCANAL_PROD_DW;
...
EXCEPTION
	WHEN OTHERS THEN
		VAR_CSIT_CTRLM		:= 5;
		VAR_LOG := 'PROBLEMA NO SUBPROGRAMA PR_POSIC_DIA_PROD_COMPL. '
				 ||' -- ERRO ORACLE: '||SUBSTR(SQLERRM, 1, 120);
		PR_GRAVA_MSG_LOG_CARGA(VAR_CROTNA, VAR_LOG, VAR_LOG_ADVERTENCIA, NULL, NULL);
		RAISE VAR_FIM_PROCESSO_ERRO;
END PR_POSIC_DIA_PROD_COMPL;
---------------------------------------------------------------------------------------------------------------------------
...
--------DADOS HIERARQUIA
		...
		WROW_HIERQ.CBCO					:= VAR_CUR_PRO_CANAL_BCO_AG.CBCO;
		WROW_HIERQ.CAG_BCRIA			:= SUBSTR(LPAD(VAR_CUR_PRO_CANAL_BCO_AG.CAG_BCRIA,7,0),4,4);
		...
		PR_CARREGA_HIERQ_PBLIC_ALVO ( 	VAR_CROTNA,							--VAR_CROTNA IN CHAR,
										...
										WROW_HIERQ.CCAMPA_DSTAQ,			--PAR_CCAMPA_DSTAQ IN NUMBER,
										VAR_CPARM_HIERQ_DSTAQ,		--VAR_CPARM_HIERQ_DSTAQ IN NUMBER,
										NULL,				--PAR_CTPO_PSSOA IN CHAR,
										NULL,			--PAR_CCPF_CNPJ_BASE IN NUMBER,
										WROW_HIERQ.CRGNAL,					--PAR_CRGNAL IN NUMBER,
										WROW_HIERQ.CBCO,								--PAR_CBCO	IN NUMBER,
										WROW_HIERQ.CAG_BCRIA,								--PAR_CAG_BCRIA IN NUMBER,
										WROW_HIERQ.CHIERQ_PBLIC_ALVO_DSTAQ,	--PAR_CHIERQ_PBLIC_ALVO_DSTAQ OUT NUMBER,
										VAR_LOG);							--PAR_ERRO_CARGA OUT CHAR)
...
BEGIN
	...
	FOR CUR_CAMPA IN (SELECT CCAMPA_DSTAQ,ICAMPA_DSTAQ,DAPURC_DSTAQ
		...
		/*IF CUR_CAMPA.CCAMPA_DSTAQ = 200801 AND VAR_DPROX_CARGA > TO_DATE('31032008','DDMMYYYY') THEN
			VAR_LOG := 'ACABOU O PROCESSO DA CAMPANHA 200801'|| VAR_CSIT_CTRLM;
			PR_GRAVA_MSG_LOG_CARGA(VAR_CROTNA,VAR_LOG,VAR_LOG_PROCESSO,NULL,NULL);
			VAR_CSIT_CTRLM		:= 5;
			RAISE VAR_FIM_PROCESSO_ERRO;
		END IF;*/
		...
		APAGA_MOVIMENTO_DIA_CANAL (CUR_CAMPA.CCAMPA_DSTAQ,8);
		VAR_TOT_REG_PROC	:= 0;
		...
		VAR_TOT_REG_PROC	:= 0;
		PR_POSIC_DIA_PROD_COMPL(CUR_CAMPA.CCAMPA_DSTAQ,VAR_DAPURC_DSTAQ,3,1);
		VAR_LOG := '--------------------TOTAL DE PRODUCAO COMPLEMENTAR EXTRA BANCO: '||VAR_TOT_REG_PROC;
		PR_GRAVA_MSG_LOG_CARGA(VAR_CROTNA,VAR_LOG,VAR_LOG_PROCESSO,NULL,NULL);

		VAR_TOT_REG_PROC	:= 0;
		PR_POSIC_DIA_PROD_COMPL(CUR_CAMPA.CCAMPA_DSTAQ,VAR_DAPURC_DSTAQ,4,2);
		VAR_LOG := '--------------------TOTAL DE PRODUCAO COMPLEMENTAR BANCO(AGENCIA): '||VAR_TOT_REG_PROC;
		PR_GRAVA_MSG_LOG_CARGA(VAR_CROTNA,VAR_LOG,VAR_LOG_PROCESSO,NULL,NULL);

		VAR_TOT_REG_PROC	:= 0;
		PR_POSIC_DIA_PROD_COMPL(CUR_CAMPA.CCAMPA_DSTAQ,VAR_DAPURC_DSTAQ,4,3);
		VAR_LOG := '--------------------TOTAL DE PRODUCAO COMPLEMENTAR BANCO(CNPJ): '||VAR_TOT_REG_PROC;
		PR_GRAVA_MSG_LOG_CARGA(VAR_CROTNA,VAR_LOG,VAR_LOG_PROCESSO,NULL,NULL);

		VAR_TOT_REG_PROC	:= 0;
		PR_POSIC_DIA_PROD_COMPL(CUR_CAMPA.CCAMPA_DSTAQ,VAR_DAPURC_DSTAQ,8,4);
		VAR_LOG := '--------------------TOTAL DE PRODUCAO COMPLEMENTAR FINASA: '||VAR_TOT_REG_PROC;
		PR_GRAVA_MSG_LOG_CARGA(VAR_CROTNA,VAR_LOG,VAR_LOG_PROCESSO,NULL,NULL);
