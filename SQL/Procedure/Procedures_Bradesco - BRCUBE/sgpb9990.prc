CREATE OR REPLACE PROCEDURE SGPB_PROC.SGPB9990 IS
  -------------------------------------------------------------------------------------------------
...
BEGIN
   ...
   --
       ...
       --DELETANDO APOLC_PROD_CRRTR
       BEGIN
         -- Esse procedimento é necessário pq esses campos abaixo são usados para marcar se a apolice já foi processada.
         -- quem depois atualiza esses campo é o programa sgpb0135 que faz a carga da produção.
         -- o programa sgpb0135 será executado nesse programa.
         UPDATE APOLC_PROD_CRRTR
         SET CGRP_RAMO_PLANO 	= NULL,
         CCOMPT_PROD          = NULL
         WHERE CCOMPT_PROD=I.COMPETENCIA; -- se desejar descobrir qual era a competencia:
         -- CCOMPT_PROD = TO_NUMBER(TO_CHAR(DEMIS_APOLC,'YYYYMM');      
       EXCEPTION
         WHEN OTHERS THEN
           var_log_erro := substr('ERRO NO UPDATE DA APOLICE. ERRO: ' || SQLERRM,1,PC_UTIL_01.VAR_TAM_MSG_ERRO);
         ROLLBACK;
       ...
       --DELETANDO PROD_CRRTR
       BEGIN
        DELETE PROD_CRRTR
         		WHERE CCOMPT_PROD = I.COMPETENCIA;
       EXCEPTION
       WHEN OTHERS THEN
