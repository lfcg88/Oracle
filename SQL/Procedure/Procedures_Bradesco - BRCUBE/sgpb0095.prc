CREATE OR REPLACE PROCEDURE SGPB_PROC.SGPB0095(
...
-------------------------------------------------------------------------------------------------
  ...
  --      objetivo        : procedure retorna lista devalores para homologação extra-banco - Publico Selecionado
  --      alterações      :
  --                data  : -
  --                autor : -
  --                obs   : -
...
--
  join crrtr_eleit_campa cec on cec.ccpf_cnpj_base =
                                     cuc.ccpf_cnpj_base
...
--
 ORDER BY "ELEITO" desc,
       "NOME CORRETORA/RAZÃO SOCIAL";
  --
END SGPB0095;
/
