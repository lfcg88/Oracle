create or replace procedure sgpb_proc.SGPB6208 is
------------------------------------------------------------------------------------------------------------------------
--  BRADESCO SEGUROS S.A.
--  DATA            : 11/10/2007
--  AUTOR           : WASSILY CHUK SEIBLITZ GUANAES
--  PROGRAMA        : SGPB6208.SQL
--  OBJETIVO        : Recalcular o Saldo de Raspadinhas Retiradas de Cada Corretor.
--
-- ---------------------------------------------------------------------------------------------------------------
--
-- ********** ATENÇÃO: ESSE PROGRAMA SOMENTE PODE SER RODADO SEM EXECUÇÃO SIMULTÂNEA COM A CARGA DIÁRIA **********
--
-- ----------------------------------------------------------------------------------------------------------------
--
------------------------------------------------------------------------------------------------------------------------
...
VAR_CROTNA                  ARQ_TRAB.CROTNA%TYPE            := 'SGPB6208';
...
-- VARIAVEIS DE TRABALHO NOVO
 ...
 VAR_DAPURC_DSTAQ           PARM_CARGA.DCARGA%TYPE;
...
BEGIN
    VAR_LOG := 'RE-CALCULO DO SALDO DE RASPADINHAS DO CORRETOR';
    COMMIT;
    BEGIN
       SELECT DAPURC_DSTAQ INTO VAR_DAPURC_DSTAQ
         	FROM CAMPA_DSTAQ
         	WHERE CCAMPA_DSTAQ = VAR_CAMPANHA;
    EXCEPTION
       WHEN OTHERS THEN
          VAR_LOG  := 'ERRO SELECT DA CAMPA_DSTAQ. ERRO ORACLE: '||SUBSTR(SQLERRM(SQLCODE), 1, 200);
          ROLLBACK;
          PR_GRAVA_MSG_LOG_CARGA( VAR_CROTNA,  VAR_LOG,  VAR_LOG_PROCESSO, NULL, NULL);
          COMMIT;
          RAISE VAR_FIM_PROCESSO_ERRO;
