CREATE OR REPLACE PROCEDURE SGPB_PROC.SGPB0135(intrCompetencia        IN date,
  ...
  BEGIN
    ...
    UPDATE prod_crrtr
     	...
     	WHERE cgrp_ramo_plano= p_cgrp_ramo_plano
       		...
       		AND ccrrtr = p_ccrrtr
       		AND ccompt_prod = to_number(TO_CHAR(intrCompetencia,'YYYYMM'))
    ...
    IF (SQL%ROWCOUNT = 0) THEN
      ...
      VALUES (p_qtot_item_prod,p_ctpo_comis,p_ccrrtr,p_cgrp_ramo_plano,to_number(TO_CHAR(intrCompetencia,'YYYYMM')),
...
BEGIN
       ...
       LOOP
        ...
        UPDATE APOLC_PROD_CRRTR
         ...
         WHERE CUND_PROD 	= C.CUND_PROD
           ...
           AND demis_apolc 	= intrCompetencia
           and CTPO_COMIS  	= C.CTPO_COMIS
           and CCRRTR      	= C.CCRRTR;
        IF (SQL%ROWCOUNT = 0) THEN
           PR_GRAVA_MSG_LOG_CARGA(CHRNOMEROTINASCHEDULER,'ERRO. O UPDATE NAO AFETOU NENHUMA LINHA.',pc_util_01.VAR_LOG_PROCESSO,NULL,NULL);
  ...
  END LOOP;
  chrLocalErro := 6;
  PR_GRAVA_MSG_LOG_CARGA(CHRNOMEROTINASCHEDULER,'FIM DA SUMARIZACAO DIARIA DA PRODUCAO DO CORRETOR.',pc_util_01.VAR_LOG_PROCESSO,NULL,NULL);
